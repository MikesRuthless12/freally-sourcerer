...
toast-search-failed = 搜索失败：{ $error }
toast-settings-save-failed = 无法保存设置：{ $error }
toast-settings-reset-failed = 无法恢复默认设置：{ $error }
toast-index-root-failed = 无法将 { $path } 添加到索引：{ $error }
menu-help-report-bug = 报告问题…
bugreport-title = 报告问题
bugreport-crash-notice = Freally Sourcerer 上次运行时意外关闭。你可以就此发送一份匿名报告。
bugreport-what-happened = 发生了什么？
bugreport-placeholder = 出问题时你正在做什么？
bugreport-include-crash = 包含下面的崩溃详情
bugreport-exact-text = 以下就是将要发送的内容
bugreport-privacy = 不会自动发送任何内容。这只会打开一封预先填好的草稿——由你阅读后自行发送。报告仅包含应用版本和你的操作系统；主目录和用户名已被移除。
bugreport-github = 在 GitHub 上开一个 issue
bugreport-gmail = 在 Gmail 中打开
bugreport-email = 在我的邮件应用中打开
bugreport-copy = 复制报告
bugreport-copied = 已复制。
bugreport-discard-crash = 丢弃此崩溃报告
bugreport-opened = 草稿已打开。请阅读后自行发送。
bugreport-failed = 无法打开草稿：{ $error }
logs-crash-drills = 崩溃报告演练
logs-simulate-crash = 模拟一份崩溃报告
logs-force-crash = 强制触发测试崩溃并退出
updates-check-now = 立即检查更新
updates-checking = 正在检查…
updates-last-checked = 上次检查
updates-never-checked = 从未
updates-current = 你正在运行最新版本（{ $current }）。
updates-available = 版本 { $version } 可用。你正在运行 { $current }。
updates-confirm-title = 安装更新
updates-confirm-body = Freally Sourcerer { $version } 可用。\n\n你正在运行 { $current }。要关闭 Freally Sourcerer 并立即安装新版本吗？
updates-confirm-yes = 是，立即安装
updates-confirm-no = 暂不
updates-installing = 正在下载并安装 { $version }……Freally Sourcerer 将会关闭。
updates-failed = 更新失败：{ $error }
updates-notes-heading = 更新内容
...
menu-tools-index-health = 索引健康状况…
health-title = 索引健康状况
health-loading = 正在读取索引健康状况…
health-all-good = 所有受监视的位置均为最新。
health-no-watched-roots = 尚未索引任何文件夹。
health-col-volume = 位置
health-col-status = 状态
health-col-lag = 延迟
health-col-last-event = 最近变更
health-col-applied = 已应用
health-col-dropped = 已丢弃
health-col-queue = 队列
health-status-live = 实时
health-status-scan-only = 仅扫描
health-lag-value = { $last } 毫秒（峰值 { $max } 毫秒）
health-no-events-yet = 暂无变更
health-never = 从未
health-ago = { $secs } 秒前
health-legend = 延迟是磁盘上的变更到出现在结果中所需的时间。“已应用”统计已索引的事件，括号内为已合并的事件。
health-extraction-untracked = 内容提取积压尚未跟踪。
health-extraction-backlog = 内容提取积压：{ $count }
health-fix-rebuild = 重建索引
health-advice-journal-stream-reset = 操作系统丢弃了 { $root } 上的变更流。其间发生的变更不在索引中——重建可将其恢复。
health-advice-events-dropped = { $root } 上有 { $count } 个变更到达速度超过索引速度而被丢弃。重建可将其恢复。
...
menu-edit-undo = 撤销
menu-edit-redo = 重做
menu-edit-bulk-rename = 重命名…
rename-title = 重命名选中的 { $count } 项
rename-find = 查找
rename-replace = 替换为
rename-use-regex = 正则表达式
rename-ignore-case = 忽略大小写
rename-part = 应用于
rename-part-stem = 不含扩展名的名称
rename-part-full = 整个文件名
rename-case = 大小写
rename-case-none = 不更改
rename-case-lower = 小写
rename-case-upper = 大写
rename-case-title = 每个单词首字母大写
rename-counter-start = 计数起始
rename-hint = 使用 { "{" }n{ "}" } 插入序号，{ "{" }n:03{ "}" } 补零。启用正则表达式时，$1 可插入捕获组。
rename-col-before = 之前
rename-col-after = 之后
rename-col-status = 状态
rename-status-ok = 将被重命名
rename-status-unchanged = 未更改
rename-status-invalid = 名称无效
rename-status-collision = 两个文件会同名
rename-status-exists = 已存在同名文件
rename-invalid-empty = 名称将为空
rename-invalid-path-separator = 名称不能包含文件夹分隔符
rename-invalid-dot-name = 名称不能为 . 或 ..
rename-invalid-forbidden-character = 名称包含不允许的字符
rename-invalid-reserved-name = 该名称被操作系统保留
rename-invalid-bad-pattern = 搜索模式无效
rename-summary = 将重命名 { $count } 个文件。
rename-blocked = 请先修正高亮的行。在此之前不会更改任何内容。
rename-apply = 重命名
rename-toast-done = 已重命名 { $count } 个文件
rename-nothing-selected = 请先选择一些结果。
ops-toast-undone = 已撤销 — 恢复了 { $count } 个文件
ops-toast-redone = 已重做 — 重新重命名了 { $count } 个文件
about-portable-on = 便携模式已开启
about-portable-hint = 索引、设置和书签都保留在此文件夹中。不会向这台电脑写入任何内容。
settings-results-natural-sort = 自然排序（file2 排在 file10 之前）
menu-search-ignore-punctuation = 忽略标点符号
menu-search-ignore-whitespace = 忽略空格
regex-builder-title = 正则表达式构建器
regex-builder-placeholder = 输入一个模式，例如 ^report-\d+
regex-builder-match-count = 显示的 { $total } 个名称中有 { $matched } 个匹配
regex-builder-no-results = 请先搜索——构建器会用屏幕上的名称测试你的模式。
regex-builder-cheatsheet = 速查表
regex-builder-flavor-note = Rust 语法。不支持反向引用和环视——搜索引擎不支持它们。
regex-builder-use = 使用此模式
regex-cheat-any = 任意单个字符
regex-cheat-classes = 数字、单词字符、空白
regex-cheat-set = 集合中的一个，或不在集合中
regex-cheat-repeat = 零个或多个、一个或多个、可选
regex-cheat-count = 恰好、至少，或一个范围
regex-cheat-anchor = 名称的开头和结尾
regex-cheat-alt = 二者之一
regex-cheat-inline-flag = 从这里开始忽略大小写
regex-cheat-noncapturing = 分组但不捕获
regex-cheat-escape = 转义特殊字符
quicklook-title = 快速查看
quicklook-position = 第 { $at } 项，共 { $total } 项
quicklook-hint = 方向键浏览结果 · 空格或 Esc 关闭
quicklook-open-native = 用 macOS 快速查看打开
menu-view-sidebar = 侧边栏
sidebar-title = 侧边栏
sidebar-bookmarks = 书签
sidebar-filters = 筛选
sidebar-volumes = 卷
sidebar-recent = 最近搜索
sidebar-no-bookmarks = 还没有书签
sidebar-no-volumes = 没有已索引的卷
sidebar-no-recent = 暂无
sidebar-clear-recent = 清除
menu-tools-permission-health = 权限状况…
permissions-title = 权限状况
permissions-all-clear = 你要求 Freally 索引的内容都能读取。
permissions-summary = 有 { $denied } 个文件夹因访问被拒绝而无法读取，另有 { $other } 个因其他原因失败。其中的文件不在索引中，也不会出现在结果里。
permissions-truncated = 另有 { $dropped } 项被统计但未列出。
permissions-how-to-fix = 如何解决
permissions-by-volume = 位置
permissions-group-counts = 拒绝 { $denied } · 其他 { $other }
permissions-macos-granted = Freally 已获得完全磁盘访问权限。下面的文件夹受其他机制保护——请检查它们的所有者和权限。
permissions-macos-steps = 在你授予完全磁盘访问权限之前，macOS 会保护这些文件夹。打开“隐私与安全性”→“完全磁盘访问权限”，开启 Freally，然后退出并重新打开 Freally 并重新扫描。
permissions-macos-open-settings = 打开完全磁盘访问权限设置
permissions-macos-undetermined = Freally 无法判断是否已授予完全磁盘访问权限，所以问题可能不在这里。
permissions-linux-steps = 当前用户无法读取这些路径。请检查文件夹的所有者和模式，或以你的账户能够穿越的权限挂载该文件系统。索引系统级路径可能需要后台服务以更高权限运行。
permissions-windows-steps = 某条 Windows 访问规则拒绝了你的账户。打开文件夹的“属性”→“安全”查看适用的规则，或以有读取权限的用户运行索引服务。
permissions-generic-steps = 操作系统拒绝读取这些路径。请检查它们的权限。
status-paths-skipped = 已跳过 { $count } 个文件夹
media-open-externally = 用系统播放器打开
media-play-pause = 播放或暂停
media-seek = 跳转
media-loop = 循环
media-volume = 音量
