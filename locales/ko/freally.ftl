...
toast-search-failed = 검색 실패: { $error }
toast-settings-save-failed = 설정을 저장할 수 없습니다: { $error }
toast-settings-reset-failed = 기본값을 복원할 수 없습니다: { $error }
toast-index-root-failed = { $path }을(를) 색인에 추가할 수 없습니다: { $error }
menu-help-report-bug = 버그 신고…
bugreport-title = 버그 신고
bugreport-crash-notice = 지난번 실행에서 Freally Sourcerer가 예기치 않게 종료되었습니다. 이에 대한 익명 보고서를 보낼 수 있습니다.
bugreport-what-happened = 무슨 일이 있었나요?
bugreport-placeholder = 문제가 생겼을 때 무엇을 하고 있었나요?
bugreport-include-crash = 아래 충돌 세부 정보 포함
bugreport-exact-text = 실제로 전송되는 내용입니다
bugreport-privacy = 자동으로 전송되는 것은 없습니다. 미리 채워진 초안이 열리며, 직접 읽고 보내시면 됩니다. 보고서에는 앱 버전과 운영체제만 담기며, 홈 폴더와 사용자 이름은 제거됩니다.
bugreport-github = GitHub 이슈 열기
bugreport-gmail = Gmail에서 열기
bugreport-email = 메일 앱에서 열기
bugreport-copy = 보고서 복사
bugreport-copied = 복사했습니다.
bugreport-discard-crash = 이 충돌 보고서 버리기
bugreport-opened = 초안이 열렸습니다. 확인한 뒤 직접 보내주세요.
bugreport-failed = 초안을 열 수 없습니다: { $error }
logs-crash-drills = 충돌 보고서 점검
logs-simulate-crash = 충돌 보고서 시뮬레이션
logs-force-crash = 테스트 충돌 후 종료
updates-check-now = 지금 업데이트 확인
updates-checking = 확인 중…
updates-last-checked = 마지막 확인
updates-never-checked = 없음
updates-current = 최신 릴리스({ $current })를 사용 중입니다.
updates-available = 버전 { $version }을(를) 사용할 수 있습니다. 현재 { $current }입니다.
updates-confirm-title = 업데이트 설치
updates-confirm-body = Freally Sourcerer { $version }을(를) 사용할 수 있습니다.\n\n현재 { $current }입니다. Freally Sourcerer를 닫고 새 버전을 지금 설치할까요?
updates-confirm-yes = 예, 설치합니다
updates-confirm-no = 나중에
updates-installing = { $version }을(를) 내려받아 설치하는 중… Freally Sourcerer가 종료됩니다.
updates-failed = 업데이트 실패: { $error }
updates-notes-heading = 새로운 기능
...
menu-tools-index-health = 색인 상태…
health-title = 색인 상태
health-loading = 색인 상태를 읽는 중…
health-all-good = 감시 중인 모든 위치가 최신입니다.
health-no-watched-roots = 아직 색인 중인 폴더가 없습니다.
health-col-volume = 위치
health-col-status = 상태
health-col-lag = 지연
health-col-last-event = 마지막 변경
health-col-applied = 적용됨
health-col-dropped = 누락됨
health-col-queue = 대기열
health-status-live = 실시간
health-status-scan-only = 검사만
health-lag-value = { $last } ms(최대 { $max } ms)
health-no-events-yet = 아직 변경 없음
health-never = 없음
health-ago = { $secs }초 전
health-legend = 지연은 디스크의 변경이 결과에 나타나기까지 걸리는 시간입니다. ‘적용됨’은 색인된 이벤트 수이며 괄호 안은 병합된 이벤트 수입니다.
health-extraction-untracked = 콘텐츠 추출 대기량은 아직 추적하지 않습니다.
health-extraction-backlog = 콘텐츠 추출 대기량: { $count }
health-fix-rebuild = 색인 다시 만들기
health-advice-journal-stream-reset = 운영체제가 { $root }의 변경 스트림을 버렸습니다. 그 사이의 변경은 색인에 없으며, 다시 만들면 복원됩니다.
health-advice-events-dropped = { $root }의 변경 { $count }건이 색인 속도보다 빠르게 도착해 누락되었습니다. 다시 만들면 복원됩니다.
...
menu-edit-bulk-rename = 이름 바꾸기…
rename-title = 선택한 { $count }개 이름 바꾸기
rename-find = 찾기
rename-replace = 바꿀 내용
rename-use-regex = 정규식
rename-ignore-case = 대소문자 무시
rename-part = 적용 대상
rename-part-stem = 확장자를 제외한 이름
rename-part-full = 전체 파일 이름
rename-case = 대소문자
rename-case-none = 변경 없음
rename-case-lower = 소문자
rename-case-upper = 대문자
rename-case-title = 단어 첫 글자 대문자
rename-counter-start = 번호 시작
rename-hint = { "{" }n{ "}" }은 번호를, { "{" }n:03{ "}" }은 자릿수를 채웁니다. 정규식에서는 $1로 캡처 그룹을 넣습니다.
rename-col-before = 이전
rename-col-after = 이후
rename-col-status = 상태
rename-status-ok = 이름이 바뀝니다
rename-status-unchanged = 변경 없음
rename-status-invalid = 잘못된 이름
rename-status-collision = 두 파일의 이름이 같아집니다
rename-status-exists = 같은 이름의 파일이 이미 있습니다
rename-invalid-empty = 이름이 비게 됩니다
rename-invalid-path-separator = 이름에 폴더 구분 기호를 쓸 수 없습니다
rename-invalid-dot-name = 이름을 . 또는 ..로 할 수 없습니다
rename-invalid-forbidden-character = 사용할 수 없는 문자가 있습니다
rename-invalid-reserved-name = 운영체제가 예약한 이름입니다
rename-invalid-bad-pattern = 검색 패턴이 올바르지 않습니다
rename-summary = { $count }개 파일의 이름이 바뀝니다.
rename-blocked = 표시된 행을 먼저 수정하세요. 그때까지 아무것도 변경되지 않습니다.
rename-apply = 이름 바꾸기
rename-toast-done = { $count }개 파일의 이름을 바꿨습니다
rename-nothing-selected = 먼저 결과를 선택하세요.
ops-toast-undone = 실행 취소됨 — { $count }개 파일 복원
ops-toast-redone = 다시 실행됨 — { $count }개 파일 이름 재변경
about-portable-on = 휴대용 모드가 켜져 있습니다
about-portable-hint = 색인, 설정, 북마크가 이 폴더에 유지됩니다. 이 컴퓨터에는 아무것도 기록되지 않습니다.
settings-results-natural-sort = 자연 정렬 (file10보다 file2 먼저)
menu-search-ignore-punctuation = 문장 부호 무시
menu-search-ignore-whitespace = 공백 무시
regex-builder-title = 정규식 작성기
regex-builder-placeholder = 패턴을 입력하세요. 예: ^report-\d+
regex-builder-match-count = 표시된 { $total }개 이름 중 { $matched }개 일치
regex-builder-no-results = 먼저 검색하세요 — 작성기는 화면에 보이는 이름으로 패턴을 시험합니다.
regex-builder-cheatsheet = 요약표
regex-builder-flavor-note = Rust 문법입니다. 역참조와 전방·후방 탐색은 검색 엔진이 지원하지 않습니다.
regex-builder-use = 이 패턴 사용
regex-cheat-any = 임의의 한 문자
regex-cheat-classes = 숫자, 단어 문자, 공백
regex-cheat-set = 집합 중 하나, 또는 집합에 없는 것
regex-cheat-repeat = 0회 이상, 1회 이상, 선택
regex-cheat-count = 정확히, 최소, 또는 범위
regex-cheat-anchor = 이름의 시작과 끝
regex-cheat-alt = 둘 중 하나
regex-cheat-inline-flag = 여기서부터 대소문자 무시
regex-cheat-noncapturing = 캡처 없이 묶기
regex-cheat-escape = 특수 문자 이스케이프
quicklook-title = 훑어보기
quicklook-position = { $total }개 중 { $at }번째
quicklook-hint = 화살표 키로 결과 이동 · 스페이스 또는 Esc로 닫기
quicklook-open-native = macOS 훑어보기로 열기
menu-view-sidebar = 사이드바
sidebar-title = 사이드바
sidebar-bookmarks = 북마크
sidebar-filters = 필터
sidebar-volumes = 볼륨
sidebar-recent = 최근 검색
sidebar-no-bookmarks = 아직 북마크가 없습니다
sidebar-no-volumes = 색인된 볼륨이 없습니다
sidebar-no-recent = 아직 없습니다
sidebar-clear-recent = 지우기
menu-tools-permission-health = 권한 상태…
permissions-title = 권한 상태
permissions-all-clear = 색인하도록 지정한 항목은 모두 읽을 수 있었습니다.
permissions-summary = 접근이 거부되어 { $denied }개 폴더를 읽지 못했고, { $other }개는 다른 이유로 실패했습니다. 그 안의 파일은 색인에 없으며 결과에도 나타나지 않습니다.
permissions-truncated = { $dropped }개가 더 집계되었지만 목록에는 없습니다.
permissions-how-to-fix = 해결 방법
permissions-by-volume = 위치
permissions-group-counts = 거부 { $denied } · 기타 { $other }
permissions-macos-granted = Freally에는 이미 전체 디스크 접근 권한이 있습니다. 아래 폴더는 다른 이유로 보호되고 있으니 소유자와 권한을 확인하세요.
permissions-macos-steps = 전체 디스크 접근을 허용하기 전까지 macOS가 이 폴더를 보호합니다. 개인정보 보호 및 보안 → 전체 디스크 접근을 열고 Freally를 켠 다음, Freally를 종료했다가 다시 열고 재검색하세요.
permissions-macos-open-settings = 전체 디스크 접근 설정 열기
permissions-macos-undetermined = 전체 디스크 접근이 이미 허용되었는지 확인하지 못했습니다. 원인이 다를 수 있습니다.
permissions-linux-steps = 현재 사용자로는 이 경로를 읽을 수 없습니다. 폴더의 소유자와 모드를 확인하거나, 계정이 통과할 수 있는 권한으로 파일 시스템을 마운트하세요. 시스템 전역 경로를 색인하려면 데몬을 상승된 권한으로 실행해야 할 수 있습니다.
permissions-windows-steps = Windows 접근 규칙이 이 계정을 거부합니다. 폴더의 속성 → 보안에서 어떤 규칙이 적용되는지 확인하거나, 읽을 수 있는 사용자로 색인 서비스를 실행하세요.
permissions-generic-steps = 운영체제가 이 경로 읽기를 거부했습니다. 권한을 확인하세요.
status-paths-skipped = 폴더 { $count }개 건너뜀
media-open-externally = 시스템 재생기로 열기
media-play-pause = 재생 또는 일시정지
media-seek = 탐색
media-loop = 반복
media-volume = 음량
