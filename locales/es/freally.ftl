...
dupe-group-summary = { $count } copias · { $bytes }
status-file-list = Lista de archivos: { $name }
status-file-list-truncated = Mostrando las primeras 5000 entradas
refine-label = Dentro de resultados
refine-input-label = Acotar los resultados actuales
refine-placeholder = Escribe para acotar, Intro para añadir
refine-count = { $shown } de { $total }
refine-remove-chip = Quitar el filtro { $term }
refine-clear = Limpiar
refine-close = Cerrar la barra de refinamiento
menu-search-within-results = Buscar dentro de los resultados
ctx-menu-label = Acciones del resultado
ctx-open = Abrir
ctx-reveal = Mostrar en el gestor de archivos
ctx-view-hits = Ver coincidencias en contexto
ctx-open-with = Abrir con
ctx-open-with-none = No hay aplicaciones registradas
ctx-loading = Cargando…
ctx-copy-contents = Copiar contenido
ctx-copy-as-file = Copiar como archivo
ctx-copy-path-list = Copiar lista de rutas
ctx-quote-lines = Una por línea
ctx-quote-quoted = Entre comillas
ctx-quote-space = Separadas por espacios
ctx-quote-escaped = Escapadas para shell
ctx-terminal-here = Abrir terminal aquí
verb-failed = { $verb } falló: { $error }
hitviewer-title = Coincidencias en { $name }
hitviewer-no-matches = Sin coincidencias
hitviewer-match-position = Coincidencia { $current } de { $total }
hitviewer-prev = Anterior
hitviewer-next = Siguiente
hitviewer-loading = Extrayendo…
hitviewer-extractor = Extraído por { $id }
hitviewer-truncated = Documento truncado para su visualización
hitviewer-keys = F3 siguiente · Mayús+F3 anterior · Esc cerrar
close = Cerrar
toast-dismiss = Descartar
toast-file-list-opened = Se abrió { $name } — { $count } entradas
toast-file-list-open-failed = No se pudo abrir la lista de archivos: { $error }
toast-export-done = Se exportaron { $written } resultados como { $format }
toast-export-lossy = Se exportaron { $written } de { $total } resultados como { $format }
toast-export-failed = Error al exportar: { $error }
toast-search-failed = Error al buscar: { $error }
toast-settings-save-failed = No se pudieron guardar los ajustes: { $error }
toast-settings-reset-failed = No se pudieron restaurar los valores predeterminados: { $error }
toast-index-root-failed = No se pudo añadir { $path } al índice: { $error }
menu-help-report-bug = Informar de un error…
bugreport-title = Informar de un error
bugreport-crash-notice = Freally Sourcerer se cerró inesperadamente la última vez que se ejecutó. Puedes enviar un informe anónimo al respecto.
bugreport-what-happened = ¿Qué ha pasado?
bugreport-placeholder = ¿Qué estabas haciendo cuando falló?
bugreport-include-crash = Incluir los detalles del fallo de abajo
bugreport-exact-text = Esto es exactamente lo que se enviará
bugreport-privacy = No se envía nada automáticamente. Se abrirá un borrador ya rellenado: tú lo lees y lo envías. El informe solo incluye la versión de la app y tu sistema operativo; se eliminan tu carpeta personal y tu nombre de usuario.
bugreport-github = Abrir una incidencia en GitHub
bugreport-gmail = Abrir en Gmail
bugreport-email = Abrir en mi app de correo
bugreport-copy = Copiar informe
bugreport-copied = Copiado.
bugreport-discard-crash = Descartar este informe de fallo
bugreport-opened = Hay un borrador abierto. Léelo y envíalo tú.
bugreport-failed = No se pudo abrir el borrador: { $error }
logs-crash-drills = Pruebas de informe de fallo
logs-simulate-crash = Simular un informe de fallo
logs-force-crash = Forzar un fallo de prueba y salir
updates-check-now = Buscar actualizaciones ahora
updates-checking = Comprobando…
updates-last-checked = Última comprobación
updates-never-checked = Nunca
updates-current = Estás usando la última versión ({ $current }).
updates-available = La versión { $version } está disponible. Estás usando { $current }.
updates-confirm-title = Instalar actualización
updates-confirm-body = Freally Sourcerer { $version } está disponible.\n\nEstás usando { $current }. ¿Quieres cerrar Freally Sourcerer e instalar la nueva versión ahora?
updates-confirm-yes = Sí, instalar
updates-confirm-no = Ahora no
updates-installing = Descargando e instalando { $version }… Freally Sourcerer se cerrará.
updates-failed = Error al actualizar: { $error }
updates-notes-heading = Novedades
toast-copied-paths = Se copiaron { $count } rutas
toast-copied-contents = Se copió el contenido de { $name }
toast-copied-files = Se copiaron { $count } archivos
settings-node-custom-commands = Comandos personalizados
settings-custom-commands-hint = Los comandos que añadas aquí aparecen en el menú contextual de los resultados. Usa { "{" }path{ "}" }, { "{" }dir{ "}" }, { "{" }name{ "}" }, { "{" }stem{ "}" } y { "{" }ext{ "}" } en los argumentos.
settings-custom-commands-empty = Todavía no hay comandos personalizados.
settings-custom-commands-unnamed = Comando sin nombre
settings-custom-commands-name = Nombre
settings-custom-commands-program = Programa
settings-custom-commands-args = Argumentos
settings-custom-commands-extensions = Solo para las extensiones
settings-custom-commands-add = Añadir comando
did-you-mean-label = Quizás quisiste decir
menu-search-match-phonetic = Coincidir fonética CJK

# SRC-M13 — Index Health panel + rebuild advisor
menu-tools-index-health = Estado del índice…
health-title = Estado del índice
health-loading = Leyendo el estado del índice…
health-all-good = Todas las ubicaciones vigiladas están al día.
health-no-watched-roots = Todavía no se indexa ninguna carpeta.
health-col-volume = Ubicación
health-col-status = Estado
health-col-lag = Retraso
health-col-last-event = Último cambio
health-col-applied = Aplicados
health-col-dropped = Descartados
health-col-queue = Cola
health-status-live = En vivo
health-status-scan-only = Solo escaneo
health-lag-value = { $last } ms (máx. { $max } ms)
health-no-events-yet = Aún sin cambios
health-never = Nunca
health-ago = hace { $secs } s
health-legend = El retraso es el tiempo entre un cambio en el disco y su aparición en los resultados. «Aplicados» cuenta los eventos indexados, con los eventos fusionados entre paréntesis.
health-extraction-untracked = La cola de extracción de contenido aún no se supervisa.
health-extraction-backlog = Cola de extracción de contenido: { $count }
health-fix-rebuild = Reconstruir índice
health-advice-journal-stream-reset = El sistema operativo descartó el flujo de cambios de { $root }. Los cambios ocurridos mientras tanto faltan en el índice; reconstruirlo los recupera.
health-advice-events-dropped = { $count } cambios en { $root } llegaron más rápido de lo que se podían indexar y se descartaron. Reconstruir el índice los recupera.
health-advice-not-monitoring = Los cambios en { $root } no se están vigilando, así que los resultados solo se actualizan al volver a escanear la carpeta.
health-advice-high-lag = Los cambios en { $root } tardan hasta { $count } ms en aparecer en los resultados.
health-advice-queue-saturated = La cola de cambios de { $root } está casi llena ({ $count } en espera); podrían empezar a descartarse cambios.

# SRC-M14 — offline removable-volume catalogs
results-offline-badge = sin conexión — { $name }
results-offline-badge-title = La unidad donde está este archivo no está conectada. La entrada procede del catálogo que Freally guarda de ese dispositivo.

# SRC-M15 bulk rename + SRC-M16 undo/redo
menu-edit-undo = Deshacer
menu-edit-redo = Rehacer
menu-edit-bulk-rename = Renombrar…
rename-title = Renombrar { $count } seleccionados
rename-find = Buscar
rename-replace = Reemplazar por
rename-use-regex = Expresión regular
rename-ignore-case = Ignorar mayúsculas
rename-part = Aplicar a
rename-part-stem = Nombre sin extensión
rename-part-full = Nombre completo
rename-case = Mayúsculas
rename-case-none = Sin cambios
rename-case-lower = minúsculas
rename-case-upper = MAYÚSCULAS
rename-case-title = Tipo Título
rename-counter-start = Contador desde
rename-hint = Use { "{" }n{ "}" } para un contador y { "{" }n:03{ "}" } para rellenarlo. Con expresiones regulares, $1 inserta un grupo.
rename-col-before = Antes
rename-col-after = Después
rename-col-status = Estado
rename-status-ok = Se renombrará
rename-status-unchanged = Sin cambios
rename-status-invalid = Nombre no válido
rename-status-collision = Dos archivos tendrían el mismo nombre
rename-status-exists = Ya hay un archivo con ese nombre
rename-invalid-empty = El nombre quedaría vacío
rename-invalid-path-separator = El nombre no puede contener separadores de carpeta
rename-invalid-dot-name = El nombre no puede ser . ni ..
rename-invalid-forbidden-character = El nombre contiene un carácter no permitido
rename-invalid-reserved-name = El nombre está reservado por el sistema
rename-invalid-bad-pattern = El patrón de búsqueda no es válido
rename-summary = Se renombrarán { $count } archivos.
rename-blocked = Corrija las filas marcadas antes de renombrar. Hasta entonces no se cambia nada.
rename-apply = Renombrar
rename-toast-done = { $count } archivos renombrados
rename-nothing-selected = Seleccione primero algunos resultados.
ops-toast-undone = Deshecho — { $count } archivos restaurados
ops-toast-redone = Rehecho — { $count } archivos renombrados de nuevo
about-portable-on = El modo portátil está activado
about-portable-hint = Tu índice, ajustes y marcadores permanecen en esta carpeta. No se escribe nada en este equipo.
settings-results-natural-sort = Orden natural (file2 antes que file10)
menu-search-ignore-punctuation = Ignorar puntuación
menu-search-ignore-whitespace = Ignorar espacios
regex-builder-title = Constructor de expresiones regulares
regex-builder-placeholder = Escribe un patrón, p. ej. ^informe-\d+
regex-builder-match-count = { $matched } de { $total } nombres mostrados coinciden
regex-builder-no-results = Haz una búsqueda primero: el constructor prueba tu patrón con los nombres en pantalla.
regex-builder-cheatsheet = Chuleta
regex-builder-flavor-note = Sintaxis de Rust. Sin retrorreferencias ni anticipación: el motor de búsqueda no las admite.
regex-builder-use = Usar este patrón
regex-cheat-any = Cualquier carácter
regex-cheat-classes = Dígito, carácter de palabra, espacio
regex-cheat-set = Uno del conjunto, o ninguno del conjunto
regex-cheat-repeat = Cero o más, uno o más, opcional
regex-cheat-count = Exactamente, al menos, o un intervalo
regex-cheat-anchor = Inicio y final del nombre
regex-cheat-alt = Uno u otro
regex-cheat-inline-flag = Ignorar mayúsculas desde aquí
regex-cheat-noncapturing = Agrupar sin capturar
regex-cheat-escape = Escapar un carácter especial
quicklook-title = Vista rápida
quicklook-position = { $at } de { $total }
quicklook-hint = Las flechas recorren los resultados · Espacio o Esc cierra
quicklook-open-native = Abrir en Vista rápida de macOS
menu-view-sidebar = Barra lateral
sidebar-title = Barra lateral
sidebar-bookmarks = Marcadores
sidebar-filters = Filtros
sidebar-volumes = Volúmenes
sidebar-recent = Búsquedas recientes
sidebar-no-bookmarks = Aún no hay marcadores
sidebar-no-volumes = No hay volúmenes indexados
sidebar-no-recent = Nada todavía
sidebar-clear-recent = Borrar
menu-tools-permission-health = Estado de permisos…
permissions-title = Estado de permisos
permissions-all-clear = Todo lo que pediste indexar se pudo leer.
permissions-summary = No se pudieron leer { $denied } carpetas por acceso denegado, y { $other } fallaron por otros motivos. Los archivos que contienen no están en el índice y no aparecerán en los resultados.
permissions-truncated = Se contaron { $dropped } más que no se listan.
permissions-how-to-fix = Cómo solucionarlo
permissions-by-volume = Dónde
permissions-group-counts = { $denied } denegadas · { $other } otras
permissions-macos-granted = Freally ya tiene Acceso total al disco. Las carpetas de abajo están protegidas por otra cosa: revisa su propietario y sus permisos.
permissions-macos-steps = macOS protege estas carpetas hasta que concedas Acceso total al disco. Abre Privacidad y seguridad → Acceso total al disco, activa Freally, cierra y vuelve a abrir Freally y vuelve a escanear.
permissions-macos-open-settings = Abrir ajustes de Acceso total al disco
permissions-macos-undetermined = Freally no pudo determinar si ya tiene Acceso total al disco, así que quizá no sea este el problema.
permissions-linux-steps = Tu usuario no puede leer estas rutas. Revisa el propietario y el modo de la carpeta, o monta el sistema de archivos con permisos que tu cuenta pueda recorrer. Indexar rutas del sistema puede requerir que el daemon se ejecute con privilegios elevados.
permissions-windows-steps = Una regla de acceso de Windows deniega tu cuenta. Abre Propiedades → Seguridad de la carpeta para ver qué regla se aplica, o ejecuta el servicio de índice como un usuario que pueda leerla.
permissions-generic-steps = El sistema operativo se negó a leer estas rutas. Revisa sus permisos.
status-paths-skipped = { $count } carpetas omitidas
media-open-externally = Abrir en el reproductor del sistema
media-play-pause = Reproducir o pausar
media-seek = Buscar
media-loop = Repetir
