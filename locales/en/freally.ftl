...
toast-search-failed = Search failed: { $error }
toast-settings-save-failed = Could not save settings: { $error }
toast-settings-reset-failed = Could not restore defaults: { $error }
toast-index-root-failed = Could not add { $path } to the index: { $error }
menu-help-report-bug = Report a Bug…
bugreport-title = Report a bug
bugreport-crash-notice = Freally Sourcerer closed unexpectedly last time it ran. You can send an anonymous report about it.
bugreport-what-happened = What happened?
bugreport-placeholder = What were you doing when it went wrong?
bugreport-include-crash = Include the crash details below
bugreport-exact-text = This is exactly what will be sent
bugreport-privacy = Nothing is sent automatically. This opens a pre-filled draft — you read it and click send yourself. The report carries the app version and your OS only; your home folder and username are removed.
bugreport-github = Open a GitHub issue
bugreport-gmail = Open in Gmail
bugreport-email = Open in my email app
bugreport-copy = Copy report
bugreport-copied = Copied.
bugreport-discard-crash = Discard this crash report
bugreport-opened = A draft is open. Read it, then send it yourself.
bugreport-failed = Could not open the draft: { $error }
logs-crash-drills = Crash-report drills
logs-simulate-crash = Simulate a crash report
logs-force-crash = Force a test crash and exit
updates-check-now = Check for updates now
updates-checking = Checking…
updates-last-checked = Last checked
updates-never-checked = Never
updates-current = You’re running the latest release ({ $current }).
updates-available = Version { $version } is available. You’re running { $current }.
updates-confirm-title = Install update
updates-confirm-body = Freally Sourcerer { $version } is available.\n\nYou’re running { $current }. Do you want to close Freally Sourcerer and install the new version now?
updates-confirm-yes = Yes, install it
updates-confirm-no = Not now
updates-installing = Downloading and installing { $version }… Freally Sourcerer will close.
updates-failed = Update failed: { $error }
updates-notes-heading = What’s new
...
menu-search-match-phonetic = Match CJK Phonetics

# SRC-M13 — Index Health panel + rebuild advisor
menu-tools-index-health = Index Health…
health-title = Index health
health-loading = Reading index health…
health-all-good = Every watched location is up to date.
health-no-watched-roots = No folders are being indexed yet.
health-col-volume = Location
health-col-status = Status
health-col-lag = Lag
health-col-last-event = Last change
health-col-applied = Applied
health-col-dropped = Dropped
health-col-queue = Queue
health-status-live = Live
health-status-scan-only = Scan only
health-lag-value = { $last } ms (peak { $max } ms)
health-no-events-yet = No changes yet
health-never = Never
health-ago = { $secs }s ago
health-legend = Lag is the time from a change on disk to it appearing in results. Applied counts indexed events, with merged events in parentheses.
health-extraction-untracked = Content-extraction backlog is not tracked yet.
health-extraction-backlog = Content-extraction backlog: { $count }
health-fix-rebuild = Rebuild index
health-advice-journal-stream-reset = The operating system discarded the change stream on { $root }. Changes made in the meantime are missing from the index — a rebuild restores them.
health-advice-events-dropped = { $count } changes on { $root } arrived faster than they could be indexed and were dropped. A rebuild restores them.
...
menu-edit-undo = Undo
menu-edit-redo = Redo
menu-edit-bulk-rename = Rename…
rename-title = Rename { $count } selected
rename-find = Find
rename-replace = Replace with
rename-use-regex = Regular expression
rename-ignore-case = Ignore case
rename-part = Apply to
rename-part-stem = Name without extension
rename-part-full = Whole file name
rename-case = Case
rename-case-none = Unchanged
rename-case-lower = lower case
rename-case-upper = UPPER CASE
rename-case-title = Title Case
rename-counter-start = Counter from
rename-hint = Use { "{" }n{ "}" } for a counter, { "{" }n:03{ "}" } to pad it. With regular expressions on, $1 inserts a capture group.
rename-col-before = Before
rename-col-after = After
rename-col-status = Status
rename-status-ok = Will rename
rename-status-unchanged = Unchanged
rename-status-invalid = Invalid name
rename-status-collision = Two files would share this name
rename-status-exists = A file already has this name
rename-invalid-empty = Name would be empty
rename-invalid-path-separator = Name cannot contain a folder separator
rename-invalid-dot-name = Name cannot be . or ..
rename-invalid-forbidden-character = Name contains a character files cannot have
rename-invalid-reserved-name = Name is reserved by the operating system
rename-invalid-bad-pattern = The search pattern is not valid
rename-summary = { $count } files will be renamed.
rename-blocked = Fix the highlighted rows before renaming. Nothing is changed until then.
rename-apply = Rename
rename-toast-done = Renamed { $count } files
rename-nothing-selected = Select some results first.
ops-toast-undone = Undone — { $count } files put back
ops-toast-redone = Redone — { $count } files renamed again
about-portable-on = Portable mode is on
about-portable-hint = Your index, settings and bookmarks stay in this folder. Nothing is written to this computer.
settings-results-natural-sort = Natural sort (file2 before file10)
menu-search-ignore-punctuation = Ignore Punctuation
menu-search-ignore-whitespace = Ignore Whitespace
regex-builder-title = Regular expression builder
regex-builder-placeholder = Type a pattern, e.g. ^report-\d+
regex-builder-match-count = { $matched } of { $total } shown names match
regex-builder-no-results = Run a search first — the builder tests your pattern against the names on screen.
regex-builder-cheatsheet = Cheat sheet
regex-builder-flavor-note = Rust syntax. No backreferences and no lookaround — the search engine does not support them.
regex-builder-use = Use this pattern
regex-cheat-any = Any single character
regex-cheat-classes = Digit, word character, whitespace
regex-cheat-set = One of a set, or none of a set
regex-cheat-repeat = None or more, one or more, optional
regex-cheat-count = Exactly, at least, or a range
regex-cheat-anchor = Start and end of the name
regex-cheat-alt = Either one or the other
regex-cheat-inline-flag = Ignore case from here on
regex-cheat-noncapturing = Group without capturing
regex-cheat-escape = Escape a special character
quicklook-title = Quick Look
quicklook-position = { $at } of { $total }
quicklook-hint = Arrow keys move through results · Space or Esc closes
quicklook-open-native = Open in macOS Quick Look
...
sidebar-no-bookmarks = No bookmarks yet
sidebar-no-volumes = No volumes indexed
sidebar-no-recent = Nothing yet
sidebar-clear-recent = Clear
menu-tools-permission-health = Permission Health…
permissions-title = Permission health
permissions-all-clear = Everything you asked Freally to index was readable.
permissions-summary = { $denied } folders could not be read because access was denied, and { $other } failed for other reasons. Files inside them are not in the index and will not appear in results.
permissions-truncated = { $dropped } more were counted but not listed.
permissions-how-to-fix = How to fix this
permissions-by-volume = Where
permissions-group-counts = { $denied } denied · { $other } other
permissions-macos-granted = Freally already has Full Disk Access. The folders below are protected by something else — check their ownership and permissions.
permissions-macos-steps = macOS protects these folders until you grant Full Disk Access. Open Privacy & Security → Full Disk Access, switch Freally on, then quit and reopen Freally and rescan.
permissions-macos-open-settings = Open Full Disk Access settings
permissions-macos-undetermined = Freally could not tell whether Full Disk Access is already granted, so this may not be the problem.
permissions-linux-steps = These paths are not readable by your user. Check the folder's owner and mode, or mount the filesystem with permissions your account can traverse. Indexing system-wide paths may need the daemon to run with elevated privileges.
permissions-windows-steps = A Windows access rule denies your account. Open the folder's Properties → Security to see which rule applies, or run the index service as a user that can read it.
permissions-generic-steps = The operating system refused to read these paths. Check their permissions.
status-paths-skipped = { $count } folders skipped
media-open-externally = Open in the system player
