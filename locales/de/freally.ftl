...
dupe-group-summary = { $count } Kopien · { $bytes }
status-file-list = Dateiliste: { $name }
status-file-list-truncated = Zeigt die ersten 5.000 Einträge
refine-label = In Ergebnissen
refine-input-label = Aktuelle Ergebnisse eingrenzen
refine-placeholder = Tippen zum Eingrenzen, Eingabe zum Hinzufügen
refine-count = { $shown } von { $total }
refine-remove-chip = Eingrenzung { $term } entfernen
refine-clear = Leeren
refine-close = Eingrenzungsleiste schließen
menu-search-within-results = In Ergebnissen suchen
ctx-menu-label = Aktionen für das Ergebnis
ctx-open = Öffnen
ctx-reveal = Im Dateimanager anzeigen
ctx-view-hits = Treffer im Kontext anzeigen
ctx-open-with = Öffnen mit
ctx-open-with-none = Keine registrierten Anwendungen
ctx-loading = Wird geladen…
ctx-copy-contents = Inhalt kopieren
ctx-copy-as-file = Als Datei kopieren
ctx-copy-path-list = Pfadliste kopieren
ctx-quote-lines = Eine pro Zeile
ctx-quote-quoted = In Anführungszeichen
ctx-quote-space = Durch Leerzeichen getrennt
ctx-quote-escaped = Shell-escaped
ctx-terminal-here = Terminal hier öffnen
verb-failed = { $verb } fehlgeschlagen: { $error }
hitviewer-title = Treffer in { $name }
hitviewer-no-matches = Keine Treffer
hitviewer-match-position = Treffer { $current } von { $total }
hitviewer-prev = Zurück
hitviewer-next = Weiter
hitviewer-loading = Wird extrahiert…
hitviewer-extractor = Extrahiert von { $id }
hitviewer-truncated = Dokument für die Anzeige gekürzt
hitviewer-keys = F3 weiter · Umschalt+F3 zurück · Esc schließen
close = Schließen
toast-dismiss = Ausblenden
toast-file-list-opened = { $name } geöffnet — { $count } Einträge
toast-file-list-open-failed = Dateiliste konnte nicht geöffnet werden: { $error }
toast-export-done = { $written } Ergebnisse als { $format } exportiert
toast-export-lossy = { $written } von { $total } Ergebnissen als { $format } exportiert
toast-export-failed = Export fehlgeschlagen: { $error }
toast-search-failed = Suche fehlgeschlagen: { $error }
toast-settings-save-failed = Einstellungen konnten nicht gespeichert werden: { $error }
toast-settings-reset-failed = Standardwerte konnten nicht wiederhergestellt werden: { $error }
toast-index-root-failed = { $path } konnte nicht zum Index hinzugefügt werden: { $error }
menu-help-report-bug = Fehler melden…
bugreport-title = Fehler melden
bugreport-crash-notice = Freally Sourcerer wurde beim letzten Start unerwartet beendet. Sie können dazu einen anonymen Bericht senden.
bugreport-what-happened = Was ist passiert?
bugreport-placeholder = Was haben Sie gerade getan, als es schiefging?
bugreport-include-crash = Absturzdetails unten einschließen
bugreport-exact-text = Genau das wird gesendet
bugreport-privacy = Es wird nichts automatisch gesendet. Es öffnet sich ein vorausgefüllter Entwurf — Sie lesen ihn und senden ihn selbst. Der Bericht enthält nur die App-Version und Ihr Betriebssystem; Ihr Benutzerordner und Benutzername werden entfernt.
bugreport-github = GitHub-Issue öffnen
bugreport-gmail = In Gmail öffnen
bugreport-email = In meiner E-Mail-App öffnen
bugreport-copy = Bericht kopieren
bugreport-copied = Kopiert.
bugreport-discard-crash = Diesen Absturzbericht verwerfen
bugreport-opened = Ein Entwurf ist geöffnet. Lesen Sie ihn und senden Sie ihn selbst.
bugreport-failed = Entwurf konnte nicht geöffnet werden: { $error }
logs-crash-drills = Absturzbericht-Übungen
logs-simulate-crash = Absturzbericht simulieren
logs-force-crash = Testabsturz erzwingen und beenden
updates-check-now = Jetzt nach Updates suchen
updates-checking = Wird gesucht…
updates-last-checked = Zuletzt geprüft
updates-never-checked = Nie
updates-current = Sie verwenden die neueste Version ({ $current }).
updates-available = Version { $version } ist verfügbar. Sie verwenden { $current }.
updates-confirm-title = Update installieren
updates-confirm-body = Freally Sourcerer { $version } ist verfügbar.\n\nSie verwenden { $current }. Möchten Sie Freally Sourcerer schließen und die neue Version jetzt installieren?
updates-confirm-yes = Ja, installieren
updates-confirm-no = Jetzt nicht
updates-installing = { $version } wird heruntergeladen und installiert… Freally Sourcerer wird geschlossen.
updates-failed = Update fehlgeschlagen: { $error }
updates-notes-heading = Neuerungen
toast-copied-paths = { $count } Pfade kopiert
toast-copied-contents = Inhalt von { $name } kopiert
toast-copied-files = { $count } Dateien kopiert
settings-node-custom-commands = Eigene Befehle
settings-custom-commands-hint = Hier hinzugefügte Befehle erscheinen im Kontextmenü der Ergebnisse. Verwende { "{" }path{ "}" }, { "{" }dir{ "}" }, { "{" }name{ "}" }, { "{" }stem{ "}" } und { "{" }ext{ "}" } in den Argumenten.
settings-custom-commands-empty = Noch keine eigenen Befehle.
settings-custom-commands-unnamed = Unbenannter Befehl
settings-custom-commands-name = Name
settings-custom-commands-program = Programm
settings-custom-commands-args = Argumente
...
menu-tools-index-health = Indexzustand…
health-title = Indexzustand
health-loading = Indexzustand wird gelesen…
health-all-good = Alle überwachten Orte sind aktuell.
health-no-watched-roots = Es werden noch keine Ordner indiziert.
health-col-volume = Ort
health-col-status = Status
health-col-lag = Verzögerung
health-col-last-event = Letzte Änderung
health-col-applied = Übernommen
health-col-dropped = Verworfen
health-col-queue = Warteschlange
health-status-live = Live
health-status-scan-only = Nur Scan
health-lag-value = { $last } ms (Spitze { $max } ms)
health-no-events-yet = Noch keine Änderungen
health-never = Nie
health-ago = vor { $secs } s
health-legend = Die Verzögerung ist die Zeit von einer Änderung auf der Festplatte bis zu ihrem Erscheinen in den Ergebnissen. „Übernommen“ zählt indizierte Ereignisse, zusammengefasste Ereignisse in Klammern.
health-extraction-untracked = Der Rückstand bei der Inhaltsextraktion wird noch nicht erfasst.
health-extraction-backlog = Rückstand bei der Inhaltsextraktion: { $count }
health-fix-rebuild = Index neu aufbauen
health-advice-journal-stream-reset = Das Betriebssystem hat den Änderungsstrom auf { $root } verworfen. Zwischenzeitliche Änderungen fehlen im Index — ein Neuaufbau stellt sie wieder her.
health-advice-events-dropped = { $count } Änderungen auf { $root } trafen schneller ein, als sie indiziert werden konnten, und wurden verworfen. Ein Neuaufbau stellt sie wieder her.
health-advice-not-monitoring = Änderungen auf { $root } werden nicht verfolgt; die Ergebnisse aktualisieren sich nur bei einem erneuten Scan.
health-advice-high-lag = Änderungen auf { $root } brauchen bis zu { $count } ms, um in den Ergebnissen zu erscheinen.
health-advice-queue-saturated = Die Änderungswarteschlange für { $root } ist fast voll ({ $count } warten); Änderungen könnten verworfen werden.

# SRC-M14 — offline removable-volume catalogs
results-offline-badge = offline — { $name }
results-offline-badge-title = Das Laufwerk mit dieser Datei ist nicht angeschlossen. Der Eintrag stammt aus Freallys Katalog dieses Geräts.

# SRC-M15 bulk rename + SRC-M16 undo/redo
menu-edit-undo = Rückgängig
menu-edit-redo = Wiederholen
menu-edit-bulk-rename = Umbenennen…
rename-title = { $count } ausgewählte umbenennen
rename-find = Suchen
rename-replace = Ersetzen durch
rename-use-regex = Regulärer Ausdruck
rename-ignore-case = Groß-/Kleinschreibung ignorieren
rename-part = Anwenden auf
rename-part-stem = Name ohne Erweiterung
rename-part-full = Ganzer Dateiname
rename-case = Schreibweise
rename-case-none = Unverändert
rename-case-lower = kleinbuchstaben
rename-case-upper = GROSSBUCHSTABEN
rename-case-title = Erster Buchstabe Groß
rename-counter-start = Zähler ab
rename-hint = { "{" }n{ "}" } fügt einen Zähler ein, { "{" }n:03{ "}" } füllt ihn auf. Mit regulären Ausdrücken fügt $1 eine Gruppe ein.
rename-col-before = Vorher
rename-col-after = Nachher
rename-col-status = Status
rename-status-ok = Wird umbenannt
rename-status-unchanged = Unverändert
rename-status-invalid = Ungültiger Name
rename-status-collision = Zwei Dateien hätten denselben Namen
rename-status-exists = Eine Datei hat diesen Namen bereits
rename-invalid-empty = Name wäre leer
rename-invalid-path-separator = Name darf kein Ordnertrennzeichen enthalten
rename-invalid-dot-name = Name darf nicht . oder .. sein
rename-invalid-forbidden-character = Name enthält ein für Dateien unzulässiges Zeichen
rename-invalid-reserved-name = Name ist vom Betriebssystem reserviert
rename-invalid-bad-pattern = Das Suchmuster ist ungültig
rename-summary = { $count } Dateien werden umbenannt.
rename-blocked = Beheben Sie die markierten Zeilen. Bis dahin wird nichts geändert.
rename-apply = Umbenennen
rename-toast-done = { $count } Dateien umbenannt
rename-nothing-selected = Wählen Sie zuerst Ergebnisse aus.
ops-toast-undone = Rückgängig — { $count } Dateien zurückgesetzt
ops-toast-redone = Wiederholt — { $count } Dateien erneut umbenannt
about-portable-on = Portabler Modus ist aktiv
about-portable-hint = Index, Einstellungen und Lesezeichen bleiben in diesem Ordner. Auf diesem Computer wird nichts gespeichert.
settings-results-natural-sort = Natürliche Sortierung (file2 vor file10)
menu-search-ignore-punctuation = Satzzeichen ignorieren
menu-search-ignore-whitespace = Leerzeichen ignorieren
regex-builder-title = Baukasten für reguläre Ausdrücke
regex-builder-placeholder = Muster eingeben, z. B. ^bericht-\d+
regex-builder-match-count = { $matched } von { $total } angezeigten Namen passen
regex-builder-no-results = Suche zuerst etwas — der Baukasten testet dein Muster an den angezeigten Namen.
regex-builder-cheatsheet = Spickzettel
regex-builder-flavor-note = Rust-Syntax. Keine Rückverweise und kein Lookaround — die Suchmaschine unterstützt sie nicht.
regex-builder-use = Dieses Muster verwenden
regex-cheat-any = Ein beliebiges Zeichen
regex-cheat-classes = Ziffer, Wortzeichen, Leerraum
regex-cheat-set = Eines aus der Menge oder keines davon
regex-cheat-repeat = Keines oder mehr, eines oder mehr, optional
regex-cheat-count = Genau, mindestens oder ein Bereich
regex-cheat-anchor = Anfang und Ende des Namens
regex-cheat-alt = Entweder das eine oder das andere
regex-cheat-inline-flag = Ab hier Groß-/Kleinschreibung ignorieren
regex-cheat-noncapturing = Gruppieren ohne zu erfassen
regex-cheat-escape = Sonderzeichen maskieren
quicklook-title = Schnellansicht
quicklook-position = { $at } von { $total }
quicklook-hint = Pfeiltasten wechseln die Ergebnisse · Leertaste oder Esc schließt
quicklook-open-native = In der macOS-Schnellansicht öffnen
menu-view-sidebar = Seitenleiste
sidebar-title = Seitenleiste
sidebar-bookmarks = Lesezeichen
sidebar-filters = Filter
sidebar-volumes = Laufwerke
sidebar-recent = Letzte Suchen
sidebar-no-bookmarks = Noch keine Lesezeichen
sidebar-no-volumes = Keine Laufwerke indiziert
sidebar-no-recent = Noch nichts
sidebar-clear-recent = Leeren
menu-tools-permission-health = Berechtigungsstatus…
permissions-title = Berechtigungsstatus
permissions-all-clear = Alles, was Freally indizieren sollte, war lesbar.
permissions-summary = { $denied } Ordner konnten wegen verweigertem Zugriff nicht gelesen werden, { $other } schlugen aus anderen Gründen fehl. Die enthaltenen Dateien stehen nicht im Index und erscheinen nicht in den Ergebnissen.
permissions-truncated = { $dropped } weitere wurden gezählt, aber nicht aufgeführt.
permissions-how-to-fix = So beheben Sie das
permissions-by-volume = Wo
permissions-group-counts = { $denied } verweigert · { $other } andere
permissions-macos-granted = Freally hat bereits vollen Festplattenzugriff. Die Ordner unten sind durch etwas anderes geschützt — prüfen Sie Eigentümer und Berechtigungen.
permissions-macos-steps = macOS schützt diese Ordner, bis Sie vollen Festplattenzugriff gewähren. Öffnen Sie Datenschutz & Sicherheit → Festplattenvollzugriff, aktivieren Sie Freally, beenden und öffnen Sie Freally neu und starten Sie einen neuen Scan.
permissions-macos-open-settings = Einstellungen für Festplattenvollzugriff öffnen
permissions-macos-undetermined = Freally konnte nicht feststellen, ob der volle Festplattenzugriff bereits gewährt ist — das ist also möglicherweise nicht die Ursache.
permissions-linux-steps = Ihr Benutzer kann diese Pfade nicht lesen. Prüfen Sie Eigentümer und Modus des Ordners, oder hängen Sie das Dateisystem mit für Ihr Konto passierbaren Rechten ein. Für systemweite Pfade muss der Dienst eventuell mit erhöhten Rechten laufen.
permissions-windows-steps = Eine Windows-Zugriffsregel verweigert Ihrem Konto den Zugriff. Öffnen Sie Eigenschaften → Sicherheit des Ordners, um die Regel zu sehen, oder lassen Sie den Indexdienst als lesberechtigter Benutzer laufen.
permissions-generic-steps = Das Betriebssystem hat das Lesen dieser Pfade verweigert. Prüfen Sie die Berechtigungen.
status-paths-skipped = { $count } Ordner übersprungen
media-open-externally = Im Systemplayer öffnen
media-play-pause = Abspielen oder pausieren
media-seek = Suchen
media-loop = Wiederholen
