...
dupe-group-summary = { $count } kopii · { $bytes }
status-file-list = Lista plików: { $name }
status-file-list-truncated = Pokazano pierwsze 5000 pozycji
refine-label = W wynikach
refine-input-label = Zawęź bieżące wyniki
refine-placeholder = Wpisz, aby zawęzić, Enter aby dodać
refine-count = { $shown } z { $total }
refine-remove-chip = Usuń zawężenie { $term }
refine-clear = Wyczyść
refine-close = Zamknij pasek zawężania
menu-search-within-results = Szukaj w wynikach
ctx-menu-label = Akcje dla wyniku
ctx-open = Otwórz
ctx-reveal = Pokaż w menedżerze plików
ctx-view-hits = Pokaż trafienia w kontekście
ctx-open-with = Otwórz za pomocą
ctx-open-with-none = Brak zarejestrowanych aplikacji
ctx-loading = Wczytywanie…
ctx-copy-contents = Kopiuj zawartość
ctx-copy-as-file = Kopiuj jako plik
ctx-copy-path-list = Kopiuj listę ścieżek
ctx-quote-lines = Po jednej w wierszu
ctx-quote-quoted = W cudzysłowach
ctx-quote-space = Oddzielone spacjami
ctx-quote-escaped = Z escapowaniem powłoki
ctx-terminal-here = Otwórz terminal tutaj
verb-failed = Niepowodzenie { $verb }: { $error }
hitviewer-title = Trafienia w { $name }
hitviewer-no-matches = Brak trafień
hitviewer-match-position = Trafienie { $current } z { $total }
hitviewer-prev = Poprzednie
hitviewer-next = Następne
hitviewer-loading = Wyodrębnianie…
hitviewer-extractor = Wyodrębnione przez { $id }
hitviewer-truncated = Dokument skrócony na potrzeby podglądu
hitviewer-keys = F3 dalej · Shift+F3 wstecz · Esc zamknij
close = Zamknij
toast-dismiss = Odrzuć
toast-file-list-opened = Otwarto { $name } — pozycji: { $count }
toast-file-list-open-failed = Nie udało się otworzyć listy plików: { $error }
toast-export-done = Wyeksportowano { $written } wyników jako { $format }
toast-export-lossy = Wyeksportowano { $written } z { $total } wyników jako { $format }
toast-export-failed = Eksport nie powiódł się: { $error }
toast-search-failed = Wyszukiwanie nie powiodło się: { $error }
toast-settings-save-failed = Nie można zapisać ustawień: { $error }
toast-settings-reset-failed = Nie można przywrócić ustawień domyślnych: { $error }
toast-index-root-failed = Nie można dodać { $path } do indeksu: { $error }
menu-help-report-bug = Zgłoś błąd…
bugreport-title = Zgłoś błąd
bugreport-crash-notice = Freally Sourcerer zamknął się nieoczekiwanie przy ostatnim uruchomieniu. Możesz wysłać anonimowe zgłoszenie na ten temat.
bugreport-what-happened = Co się stało?
bugreport-placeholder = Co robiłeś, gdy wystąpił problem?
bugreport-include-crash = Dołącz poniższe szczegóły awarii
bugreport-exact-text = Dokładnie to zostanie wysłane
bugreport-privacy = Nic nie jest wysyłane automatycznie. Otworzy się wstępnie wypełniona wiadomość — czytasz ją i wysyłasz samodzielnie. Zgłoszenie zawiera tylko wersję aplikacji i system operacyjny; folder domowy i nazwa użytkownika są usuwane.
bugreport-github = Otwórz zgłoszenie na GitHubie
bugreport-gmail = Otwórz w Gmailu
bugreport-email = Otwórz w mojej aplikacji pocztowej
bugreport-copy = Kopiuj zgłoszenie
bugreport-copied = Skopiowano.
bugreport-discard-crash = Odrzuć ten raport awarii
bugreport-opened = Wiadomość jest otwarta. Przeczytaj ją i wyślij samodzielnie.
bugreport-failed = Nie można otworzyć wiadomości: { $error }
logs-crash-drills = Próby raportu awarii
logs-simulate-crash = Symuluj raport awarii
logs-force-crash = Wymuś awarię testową i zakończ
updates-check-now = Sprawdź aktualizacje teraz
updates-checking = Sprawdzanie…
updates-last-checked = Ostatnio sprawdzono
updates-never-checked = Nigdy
updates-current = Używasz najnowszego wydania ({ $current }).
updates-available = Dostępna jest wersja { $version }. Używasz { $current }.
updates-confirm-title = Zainstaluj aktualizację
updates-confirm-body = Freally Sourcerer { $version } jest dostępny.\n\nUżywasz { $current }. Czy zamknąć Freally Sourcerer i zainstalować nową wersję teraz?
updates-confirm-yes = Tak, zainstaluj
updates-confirm-no = Nie teraz
updates-installing = Pobieranie i instalowanie { $version }… Freally Sourcerer zostanie zamknięty.
updates-failed = Aktualizacja nie powiodła się: { $error }
updates-notes-heading = Co nowego
toast-copied-paths = Skopiowano { $count } ścieżek
toast-copied-contents = Skopiowano zawartość { $name }
toast-copied-files = Skopiowano { $count } plików
settings-node-custom-commands = Polecenia niestandardowe
settings-custom-commands-hint = Polecenia dodane tutaj pojawiają się w menu kontekstowym wyników. Użyj { "{" }path{ "}" }, { "{" }dir{ "}" }, { "{" }name{ "}" }, { "{" }stem{ "}" } i { "{" }ext{ "}" } w argumentach.
settings-custom-commands-empty = Brak poleceń niestandardowych.
settings-custom-commands-unnamed = Polecenie bez nazwy
settings-custom-commands-name = Nazwa
settings-custom-commands-program = Program
settings-custom-commands-args = Argumenty
settings-custom-commands-extensions = Tylko dla rozszerzeń
settings-custom-commands-add = Dodaj polecenie
did-you-mean-label = Czy chodziło o
menu-search-match-phonetic = Dopasuj fonetykę CJK

# SRC-M13 — Index Health panel + rebuild advisor
menu-tools-index-health = Kondycja indeksu…
health-title = Kondycja indeksu
health-loading = Odczytywanie kondycji indeksu…
health-all-good = Wszystkie obserwowane lokalizacje są aktualne.
health-no-watched-roots = Żaden folder nie jest jeszcze indeksowany.
health-col-volume = Lokalizacja
health-col-status = Stan
health-col-lag = Opóźnienie
health-col-last-event = Ostatnia zmiana
health-col-applied = Zastosowane
health-col-dropped = Utracone
health-col-queue = Kolejka
health-status-live = Na żywo
health-status-scan-only = Tylko skanowanie
health-lag-value = { $last } ms (szczyt { $max } ms)
health-no-events-yet = Brak zmian
health-never = Nigdy
health-ago = { $secs } s temu
health-legend = Opóźnienie to czas od zmiany na dysku do jej pojawienia się w wynikach. „Zastosowane” liczy zaindeksowane zdarzenia, scalone zdarzenia w nawiasach.
health-extraction-untracked = Zaległości w wydobywaniu treści nie są jeszcze śledzone.
health-extraction-backlog = Zaległości w wydobywaniu treści: { $count }
health-fix-rebuild = Odbuduj indeks
health-advice-journal-stream-reset = System operacyjny odrzucił strumień zmian na { $root }. Zmiany z tego okresu nie ma w indeksie — odbudowa je przywróci.
health-advice-events-dropped = { $count } zmian na { $root } napłynęło szybciej, niż można je było zaindeksować, i zostało utraconych. Odbudowa je przywróci.
health-advice-not-monitoring = Zmiany na { $root } nie są śledzone, więc wyniki aktualizują się dopiero po ponownym skanowaniu.
health-advice-high-lag = Zmiany na { $root } pojawiają się w wynikach nawet po { $count } ms.
health-advice-queue-saturated = Kolejka zmian dla { $root } jest prawie pełna ({ $count } oczekuje); zmiany mogą zacząć być tracone.

# SRC-M14 — offline removable-volume catalogs
results-offline-badge = offline — { $name }
results-offline-badge-title = Dysk z tym plikiem nie jest podłączony. Wpis pochodzi z katalogu tego urządzenia zapisanego przez Freally.

# SRC-M15 bulk rename + SRC-M16 undo/redo
menu-edit-undo = Cofnij
menu-edit-redo = Ponów
menu-edit-bulk-rename = Zmień nazwę…
rename-title = Zmień nazwę { $count } zaznaczonych
rename-find = Znajdź
rename-replace = Zamień na
rename-use-regex = Wyrażenie regularne
rename-ignore-case = Ignoruj wielkość liter
rename-part = Zastosuj do
rename-part-stem = Nazwa bez rozszerzenia
rename-part-full = Cała nazwa pliku
rename-case = Wielkość liter
rename-case-none = Bez zmian
rename-case-lower = małe litery
rename-case-upper = WIELKIE LITERY
rename-case-title = Pierwsze Litery Wielkie
rename-counter-start = Licznik od
rename-hint = Użyj { "{" }n{ "}" } dla licznika, { "{" }n:03{ "}" } aby go uzupełnić. Przy wyrażeniach regularnych $1 wstawia grupę.
rename-col-before = Przed
rename-col-after = Po
rename-col-status = Stan
rename-status-ok = Zostanie zmieniona
rename-status-unchanged = Bez zmian
rename-status-invalid = Nieprawidłowa nazwa
rename-status-collision = Dwa pliki miałyby tę samą nazwę
rename-status-exists = Plik o tej nazwie już istnieje
rename-invalid-empty = Nazwa byłaby pusta
rename-invalid-path-separator = Nazwa nie może zawierać separatora folderów
rename-invalid-dot-name = Nazwa nie może być . ani ..
rename-invalid-forbidden-character = Nazwa zawiera niedozwolony znak
rename-invalid-reserved-name = Nazwa jest zarezerwowana przez system
rename-invalid-bad-pattern = Wzorzec wyszukiwania jest nieprawidłowy
rename-summary = Zostanie zmieniona nazwa { $count } plików.
rename-blocked = Popraw zaznaczone wiersze przed zmianą nazw. Do tego czasu nic nie zostanie zmienione.
rename-apply = Zmień nazwę
rename-toast-done = Zmieniono nazwy { $count } plików
rename-nothing-selected = Najpierw zaznacz wyniki.
ops-toast-undone = Cofnięto — przywrócono { $count } plików
ops-toast-redone = Ponowiono — ponownie zmieniono nazwy { $count } plików
about-portable-on = Tryb przenośny jest włączony
about-portable-hint = Indeks, ustawienia i zakładki pozostają w tym folderze. Nic nie jest zapisywane na tym komputerze.
settings-results-natural-sort = Sortowanie naturalne (file2 przed file10)
menu-search-ignore-punctuation = Ignoruj interpunkcję
menu-search-ignore-whitespace = Ignoruj spacje
regex-builder-title = Kreator wyrażeń regularnych
regex-builder-placeholder = Wpisz wzorzec, np. ^raport-\d+
regex-builder-match-count = Pasuje { $matched } z { $total } pokazanych nazw
regex-builder-no-results = Najpierw wyszukaj — kreator testuje wzorzec na nazwach widocznych na ekranie.
regex-builder-cheatsheet = Ściągawka
regex-builder-flavor-note = Składnia Rusta. Bez wstecznych odwołań i bez lookaround — silnik wyszukiwania ich nie obsługuje.
regex-builder-use = Użyj tego wzorca
regex-cheat-any = Dowolny pojedynczy znak
regex-cheat-classes = Cyfra, znak słowa, biały znak
regex-cheat-set = Jeden ze zbioru albo żaden ze zbioru
regex-cheat-repeat = Zero lub więcej, jeden lub więcej, opcjonalny
regex-cheat-count = Dokładnie, co najmniej albo zakres
regex-cheat-anchor = Początek i koniec nazwy
regex-cheat-alt = Jedno albo drugie
regex-cheat-inline-flag = Od tego miejsca ignoruj wielkość liter
regex-cheat-noncapturing = Grupuj bez przechwytywania
regex-cheat-escape = Znak specjalny jako zwykły
quicklook-title = Szybki podgląd
quicklook-position = { $at } z { $total }
quicklook-hint = Strzałki przechodzą między wynikami · Spacja lub Esc zamyka
quicklook-open-native = Otwórz w Szybkim podglądzie macOS
menu-view-sidebar = Panel boczny
sidebar-title = Panel boczny
sidebar-bookmarks = Zakładki
sidebar-filters = Filtry
sidebar-volumes = Woluminy
sidebar-recent = Ostatnie wyszukiwania
sidebar-no-bookmarks = Brak zakładek
sidebar-no-volumes = Brak zindeksowanych woluminów
sidebar-no-recent = Jeszcze nic
sidebar-clear-recent = Wyczyść
menu-tools-permission-health = Stan uprawnień…
permissions-title = Stan uprawnień
permissions-all-clear = Wszystko, co miało zostać zindeksowane, dało się odczytać.
permissions-summary = Nie udało się odczytać { $denied } folderów z powodu odmowy dostępu, a { $other } nie powiodło się z innych przyczyn. Znajdujące się w nich pliki nie są w indeksie i nie pojawią się w wynikach.
permissions-truncated = Policzono jeszcze { $dropped }, ale ich nie wypisano.
permissions-how-to-fix = Jak to naprawić
permissions-by-volume = Gdzie
permissions-group-counts = { $denied } odmów · { $other } innych
permissions-macos-granted = Freally ma już pełny dostęp do dysku. Poniższe foldery chroni coś innego — sprawdź ich właściciela i uprawnienia.
permissions-macos-steps = macOS chroni te foldery, dopóki nie przyznasz pełnego dostępu do dysku. Otwórz Prywatność i bezpieczeństwo → Pełny dostęp do dysku, włącz Freally, zamknij i uruchom Freally ponownie, a potem przeskanuj jeszcze raz.
permissions-macos-open-settings = Otwórz ustawienia pełnego dostępu do dysku
permissions-macos-undetermined = Freally nie ustalił, czy pełny dostęp do dysku już przyznano, więc przyczyna może być inna.
permissions-linux-steps = Twój użytkownik nie może odczytać tych ścieżek. Sprawdź właściciela i tryb folderu albo zamontuj system plików z uprawnieniami, przez które Twoje konto może przejść. Indeksowanie ścieżek systemowych może wymagać uruchomienia demona z podwyższonymi uprawnieniami.
permissions-windows-steps = Reguła dostępu systemu Windows odmawia Twojemu kontu. Otwórz Właściwości → Zabezpieczenia folderu, aby zobaczyć regułę, albo uruchom usługę indeksowania jako użytkownik, który może go odczytać.
permissions-generic-steps = System operacyjny odmówił odczytu tych ścieżek. Sprawdź ich uprawnienia.
status-paths-skipped = pominięto folderów: { $count }
media-open-externally = Otwórz w odtwarzaczu systemowym
media-play-pause = Odtwórz lub wstrzymaj
media-seek = Przewiń
media-loop = Zapętl
