...
toast-search-failed = فشل البحث: { $error }
toast-settings-save-failed = تعذّر حفظ الإعدادات: { $error }
toast-settings-reset-failed = تعذّرت استعادة الإعدادات الافتراضية: { $error }
toast-index-root-failed = تعذّرت إضافة { $path } إلى الفهرس: { $error }
menu-help-report-bug = الإبلاغ عن خلل…
bugreport-title = الإبلاغ عن خلل
bugreport-crash-notice = أُغلق Freally Sourcerer بشكل غير متوقع في آخر تشغيل. يمكنك إرسال تقرير مجهول عن ذلك.
bugreport-what-happened = ما الذي حدث؟
bugreport-placeholder = ماذا كنت تفعل عندما حدث الخطأ؟
bugreport-include-crash = تضمين تفاصيل التعطل أدناه
bugreport-exact-text = هذا هو النص الذي سيُرسَل تمامًا
bugreport-privacy = لا يُرسل أي شيء تلقائيًا. سيُفتح مسودة مُعبّأة مسبقًا — تقرأها وترسلها بنفسك. يحتوي التقرير على إصدار التطبيق ونظام التشغيل فقط؛ ويُزال مجلد المنزل واسم المستخدم.
bugreport-github = فتح مشكلة على GitHub
bugreport-gmail = فتح في Gmail
bugreport-email = فتح في تطبيق البريد
bugreport-copy = نسخ التقرير
bugreport-copied = تم النسخ.
bugreport-discard-crash = تجاهل تقرير التعطل هذا
bugreport-opened = المسودة مفتوحة. اقرأها ثم أرسلها بنفسك.
bugreport-failed = تعذّر فتح المسودة: { $error }
logs-crash-drills = تدريبات تقارير التعطل
logs-simulate-crash = محاكاة تقرير تعطل
logs-force-crash = فرض تعطل تجريبي والخروج
updates-check-now = التحقق من التحديثات الآن
updates-checking = جارٍ التحقق…
updates-last-checked = آخر تحقق
updates-never-checked = أبدًا
updates-current = أنت تستخدم أحدث إصدار ({ $current }).
updates-available = الإصدار { $version } متاح. أنت تستخدم { $current }.
updates-confirm-title = تثبيت التحديث
updates-confirm-body = الإصدار { $version } من Freally Sourcerer متاح.\n\nأنت تستخدم { $current }. هل تريد إغلاق Freally Sourcerer وتثبيت الإصدار الجديد الآن؟
updates-confirm-yes = نعم، ثبّته
updates-confirm-no = ليس الآن
updates-installing = جارٍ تنزيل وتثبيت { $version }… سيُغلق Freally Sourcerer.
updates-failed = فشل التحديث: { $error }
updates-notes-heading = الجديد
...
menu-tools-index-health = حالة الفهرس…
health-title = حالة الفهرس
health-loading = جارٍ قراءة حالة الفهرس…
health-all-good = كل المواقع المراقَبة محدَّثة.
health-no-watched-roots = لا تتم فهرسة أي مجلد بعد.
health-col-volume = الموقع
health-col-status = الحالة
health-col-lag = التأخر
health-col-last-event = آخر تغيير
health-col-applied = المطبَّق
health-col-dropped = المُهمَل
health-col-queue = الطابور
health-status-live = مباشر
health-status-scan-only = فحص فقط
health-lag-value = { $last } مللي ثانية (الذروة { $max } مللي ثانية)
health-no-events-yet = لا تغييرات بعد
health-never = أبدًا
health-ago = قبل { $secs } ثانية
health-legend = التأخر هو الزمن بين حدوث تغيير على القرص وظهوره في النتائج. «المطبَّق» يَعُدّ الأحداث المفهرسة، والأحداث المدمجة بين قوسين.
health-extraction-untracked = لا يجري بعد تتبع تراكم استخراج المحتوى.
health-extraction-backlog = تراكم استخراج المحتوى: { $count }
health-fix-rebuild = إعادة بناء الفهرس
health-advice-journal-stream-reset = تخلّص نظام التشغيل من تدفق التغييرات على { $root }. التغييرات التي حدثت في تلك الأثناء غير موجودة في الفهرس — وإعادة البناء تستعيدها.
health-advice-events-dropped = وصل { $count } تغييرًا على { $root } أسرع مما يمكن فهرسته فأُهمِلت. إعادة البناء تستعيدها.
...
menu-edit-bulk-rename = إعادة تسمية…
rename-title = إعادة تسمية { $count } عنصرًا محددًا
rename-find = بحث
rename-replace = استبدال بـ
rename-use-regex = تعبير نمطي
rename-ignore-case = تجاهل حالة الأحرف
rename-part = تطبيق على
rename-part-stem = الاسم دون الامتداد
rename-part-full = اسم الملف كاملاً
rename-case = حالة الأحرف
rename-case-none = دون تغيير
rename-case-lower = أحرف صغيرة
rename-case-upper = أحرف كبيرة
rename-case-title = أول كل كلمة كبير
rename-counter-start = بدء العدّاد من
rename-hint = استخدم { "{" }n{ "}" } لعدّاد و{ "{" }n:03{ "}" } لملئه بالأصفار. مع التعابير النمطية يُدرج ‎$1‎ مجموعة التقاط.
rename-col-before = قبل
rename-col-after = بعد
rename-col-status = الحالة
rename-status-ok = ستتم إعادة التسمية
rename-status-unchanged = دون تغيير
rename-status-invalid = اسم غير صالح
rename-status-collision = سيحمل ملفان الاسم نفسه
rename-status-exists = يوجد ملف بهذا الاسم بالفعل
rename-invalid-empty = سيصبح الاسم فارغًا
rename-invalid-path-separator = لا يمكن أن يحتوي الاسم على فاصل مجلدات
rename-invalid-dot-name = لا يمكن أن يكون الاسم . أو ..
rename-invalid-forbidden-character = يحتوي الاسم على حرف غير مسموح به
rename-invalid-reserved-name = الاسم محجوز من نظام التشغيل
rename-invalid-bad-pattern = نمط البحث غير صالح
rename-summary = ستتم إعادة تسمية { $count } ملفًا.
rename-blocked = صحّح الصفوف المميّزة قبل إعادة التسمية. لن يتغيّر شيء حتى ذلك الحين.
rename-apply = إعادة تسمية
rename-toast-done = أُعيدت تسمية { $count } ملفًا
rename-nothing-selected = اختر بعض النتائج أولاً.
ops-toast-undone = تم التراجع — أُعيد { $count } ملفًا
ops-toast-redone = تمت الإعادة — أُعيدت تسمية { $count } ملفًا
about-portable-on = الوضع المحمول مُفعَّل
about-portable-hint = يبقى الفهرس والإعدادات والإشارات المرجعية في هذا المجلد. لا يُكتب أي شيء على هذا الحاسوب.
settings-results-natural-sort = ترتيب طبيعي (‏file2 قبل ‏file10)
menu-search-ignore-punctuation = تجاهل علامات الترقيم
menu-search-ignore-whitespace = تجاهل المسافات
regex-builder-title = منشئ التعبيرات النمطية
regex-builder-placeholder = اكتب نمطًا، مثل ‏^تقرير-\d+
regex-builder-match-count = يطابق { $matched } من { $total } من الأسماء المعروضة
regex-builder-no-results = ابحث أولًا — يختبر المنشئ نمطك على الأسماء الظاهرة على الشاشة.
regex-builder-cheatsheet = ورقة مرجعية
regex-builder-flavor-note = صياغة Rust. لا مراجع خلفية ولا نظر أمامي أو خلفي — محرك البحث لا يدعمها.
regex-builder-use = استخدم هذا النمط
regex-cheat-any = أي حرف واحد
regex-cheat-classes = رقم، حرف كلمة، مسافة
regex-cheat-set = واحد من المجموعة، أو لا شيء منها
regex-cheat-repeat = صفر أو أكثر، واحد أو أكثر، اختياري
regex-cheat-count = بالضبط، على الأقل، أو مدى
regex-cheat-anchor = بداية الاسم ونهايته
regex-cheat-alt = أحدهما أو الآخر
regex-cheat-inline-flag = تجاهل حالة الأحرف من هنا
regex-cheat-noncapturing = تجميع دون التقاط
regex-cheat-escape = تخليص حرف خاص
quicklook-title = نظرة سريعة
quicklook-position = { $at } من { $total }
quicklook-hint = مفاتيح الأسهم تتنقل بين النتائج · المسافة أو Esc للإغلاق
quicklook-open-native = افتح في نظرة سريعة من macOS
menu-view-sidebar = الشريط الجانبي
sidebar-title = الشريط الجانبي
sidebar-bookmarks = الإشارات المرجعية
sidebar-filters = عوامل التصفية
sidebar-volumes = وحدات التخزين
sidebar-recent = عمليات البحث الأخيرة
sidebar-no-bookmarks = لا توجد إشارات مرجعية بعد
sidebar-no-volumes = لا توجد وحدات تخزين مفهرسة
sidebar-no-recent = لا شيء بعد
sidebar-clear-recent = مسح
menu-tools-permission-health = حالة الأذونات…
permissions-title = حالة الأذونات
permissions-all-clear = كل ما طلبت من Freally فهرسته كان قابلًا للقراءة.
permissions-summary = تعذّرت قراءة { $denied } مجلدًا بسبب رفض الوصول، وأخفق { $other } لأسباب أخرى. الملفات داخلها ليست في الفهرس ولن تظهر في النتائج.
permissions-truncated = وجرى إحصاء { $dropped } أخرى دون إدراجها.
permissions-how-to-fix = كيفية الإصلاح
permissions-by-volume = أين
permissions-group-counts = { $denied } مرفوض · { $other } غير ذلك
permissions-macos-granted = لدى Freally بالفعل وصول كامل إلى القرص. المجلدات أدناه محمية بشيء آخر — تحقّق من مالكها وأذوناتها.
permissions-macos-steps = يحمي macOS هذه المجلدات حتى تمنح الوصول الكامل إلى القرص. افتح الخصوصية والأمان ← الوصول الكامل إلى القرص، فعّل Freally، ثم أغلق Freally وأعد فتحه وأعد الفحص.
permissions-macos-open-settings = افتح إعدادات الوصول الكامل إلى القرص
permissions-macos-undetermined = لم يستطع Freally تحديد ما إذا كان الوصول الكامل إلى القرص ممنوحًا بالفعل، لذا قد لا تكون هذه هي المشكلة.
permissions-linux-steps = لا يستطيع مستخدمك قراءة هذه المسارات. تحقّق من مالك المجلد ووضعه، أو ركّب نظام الملفات بأذونات يستطيع حسابك اجتيازها. قد تتطلب فهرسة مسارات النظام تشغيل الخدمة بامتيازات مرتفعة.
permissions-windows-steps = قاعدة وصول في Windows ترفض حسابك. افتح خصائص المجلد ← الأمان لمعرفة القاعدة المطبَّقة، أو شغّل خدمة الفهرسة بمستخدم يستطيع قراءتها.
permissions-generic-steps = رفض نظام التشغيل قراءة هذه المسارات. تحقّق من أذوناتها.
status-paths-skipped = تم تخطي { $count } مجلدًا
media-open-externally = افتح في مشغل النظام
media-play-pause = تشغيل أو إيقاف مؤقت
media-seek = تقديم
media-loop = تكرار
