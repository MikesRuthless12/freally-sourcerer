...
dupe-group-summary = { $count } копий · { $bytes }
status-file-list = Список файлов: { $name }
status-file-list-truncated = Показаны первые 5000 записей
refine-label = В результатах
refine-input-label = Сузить текущие результаты
refine-placeholder = Введите, чтобы сузить, Enter — добавить
refine-count = { $shown } из { $total }
refine-remove-chip = Убрать уточнение { $term }
refine-clear = Очистить
refine-close = Закрыть панель уточнения
menu-search-within-results = Искать в результатах
ctx-menu-label = Действия с результатом
ctx-open = Открыть
ctx-reveal = Показать в файловом менеджере
ctx-view-hits = Показать совпадения в контексте
ctx-open-with = Открыть с помощью
ctx-open-with-none = Нет зарегистрированных приложений
ctx-loading = Загрузка…
ctx-copy-contents = Копировать содержимое
ctx-copy-as-file = Копировать как файл
ctx-copy-path-list = Копировать список путей
ctx-quote-lines = По одному в строке
ctx-quote-quoted = В кавычках
ctx-quote-space = Через пробел
ctx-quote-escaped = С экранированием для оболочки
ctx-terminal-here = Открыть терминал здесь
verb-failed = Сбой { $verb }: { $error }
hitviewer-title = Совпадения в { $name }
hitviewer-no-matches = Совпадений нет
hitviewer-match-position = Совпадение { $current } из { $total }
hitviewer-prev = Назад
hitviewer-next = Вперёд
hitviewer-loading = Извлечение…
hitviewer-extractor = Извлечено средством { $id }
hitviewer-truncated = Документ сокращён для показа
hitviewer-keys = F3 далее · Shift+F3 назад · Esc закрыть
close = Закрыть
toast-dismiss = Скрыть
toast-file-list-opened = Открыт { $name } — записей: { $count }
toast-file-list-open-failed = Не удалось открыть список файлов: { $error }
toast-export-done = Экспортировано результатов: { $written } в формате { $format }
toast-export-lossy = Экспортировано { $written } из { $total } результатов в формате { $format }
toast-export-failed = Сбой экспорта: { $error }
toast-search-failed = Сбой поиска: { $error }
toast-settings-save-failed = Не удалось сохранить настройки: { $error }
toast-settings-reset-failed = Не удалось восстановить значения по умолчанию: { $error }
toast-index-root-failed = Не удалось добавить { $path } в индекс: { $error }
menu-help-report-bug = Сообщить об ошибке…
bugreport-title = Сообщить об ошибке
bugreport-crash-notice = В прошлый раз Freally Sourcerer завершился неожиданно. Вы можете отправить анонимный отчёт об этом.
bugreport-what-happened = Что произошло?
bugreport-placeholder = Что вы делали, когда возникла проблема?
bugreport-include-crash = Включить приведённые ниже сведения о сбое
bugreport-exact-text = Именно это и будет отправлено
bugreport-privacy = Ничего не отправляется автоматически. Откроется заполненный черновик — вы прочитаете его и отправите сами. В отчёте только версия приложения и ваша ОС; домашняя папка и имя пользователя удаляются.
bugreport-github = Открыть issue на GitHub
bugreport-gmail = Открыть в Gmail
bugreport-email = Открыть в почтовом клиенте
bugreport-copy = Копировать отчёт
bugreport-copied = Скопировано.
bugreport-discard-crash = Удалить этот отчёт о сбое
bugreport-opened = Черновик открыт. Прочитайте его и отправьте сами.
bugreport-failed = Не удалось открыть черновик: { $error }
logs-crash-drills = Проверки отчёта о сбое
logs-simulate-crash = Смоделировать отчёт о сбое
logs-force-crash = Принудительно вызвать тестовый сбой и выйти
updates-check-now = Проверить обновления сейчас
updates-checking = Проверка…
updates-last-checked = Последняя проверка
updates-never-checked = Никогда
updates-current = У вас последняя версия ({ $current }).
updates-available = Доступна версия { $version }. У вас { $current }.
updates-confirm-title = Установить обновление
updates-confirm-body = Доступен Freally Sourcerer { $version }.\n\nУ вас { $current }. Закрыть Freally Sourcerer и установить новую версию сейчас?
updates-confirm-yes = Да, установить
updates-confirm-no = Не сейчас
updates-installing = Загрузка и установка { $version }… Freally Sourcerer будет закрыт.
updates-failed = Не удалось обновить: { $error }
updates-notes-heading = Что нового
toast-copied-paths = Скопировано путей: { $count }
toast-copied-contents = Скопировано содержимое { $name }
toast-copied-files = Скопировано файлов: { $count }
settings-node-custom-commands = Пользовательские команды
settings-custom-commands-hint = Добавленные здесь команды появляются в контекстном меню результатов. Используйте { "{" }path{ "}" }, { "{" }dir{ "}" }, { "{" }name{ "}" }, { "{" }stem{ "}" } и { "{" }ext{ "}" } в аргументах.
settings-custom-commands-empty = Пользовательских команд пока нет.
settings-custom-commands-unnamed = Команда без имени
settings-custom-commands-name = Название
settings-custom-commands-program = Программа
...
menu-tools-index-health = Состояние индекса…
health-title = Состояние индекса
health-loading = Чтение состояния индекса…
health-all-good = Все отслеживаемые расположения актуальны.
health-no-watched-roots = Ни одна папка ещё не индексируется.
health-col-volume = Расположение
health-col-status = Состояние
health-col-lag = Задержка
health-col-last-event = Последнее изменение
health-col-applied = Применено
health-col-dropped = Потеряно
health-col-queue = Очередь
health-status-live = В реальном времени
health-status-scan-only = Только сканирование
health-lag-value = { $last } мс (пик { $max } мс)
health-no-events-yet = Изменений пока нет
health-never = Никогда
health-ago = { $secs } с назад
health-legend = Задержка — время от изменения на диске до его появления в результатах. «Применено» считает проиндексированные события, объединённые события указаны в скобках.
health-extraction-untracked = Очередь извлечения содержимого пока не отслеживается.
health-extraction-backlog = Очередь извлечения содержимого: { $count }
health-fix-rebuild = Перестроить индекс
health-advice-journal-stream-reset = Операционная система сбросила поток изменений на { $root }. Изменения за это время отсутствуют в индексе — перестроение вернёт их.
health-advice-events-dropped = { $count } изменений на { $root } поступили быстрее, чем их удавалось проиндексировать, и были потеряны. Перестроение вернёт их.
health-advice-not-monitoring = Изменения на { $root } не отслеживаются, поэтому результаты обновляются только при повторном сканировании.
health-advice-high-lag = Изменения на { $root } появляются в результатах с задержкой до { $count } мс.
health-advice-queue-saturated = Очередь изменений для { $root } почти заполнена ({ $count } в ожидании); изменения могут начать теряться.

# SRC-M14 — offline removable-volume catalogs
results-offline-badge = не подключён — { $name }
results-offline-badge-title = Диск с этим файлом не подключён. Запись взята из каталога этого устройства, сохранённого Freally.

# SRC-M15 bulk rename + SRC-M16 undo/redo
menu-edit-undo = Отменить
menu-edit-redo = Повторить
menu-edit-bulk-rename = Переименовать…
rename-title = Переименовать { $count } выбранных
rename-find = Найти
rename-replace = Заменить на
rename-use-regex = Регулярное выражение
rename-ignore-case = Игнорировать регистр
rename-part = Применить к
rename-part-stem = Имя без расширения
rename-part-full = Всё имя файла
rename-case = Регистр
rename-case-none = Без изменений
rename-case-lower = нижний регистр
rename-case-upper = ВЕРХНИЙ РЕГИСТР
rename-case-title = Каждое Слово С Заглавной
rename-counter-start = Счётчик с
rename-hint = { "{" }n{ "}" } вставляет счётчик, { "{" }n:03{ "}" } дополняет его нулями. С регулярными выражениями $1 вставляет группу.
rename-col-before = До
rename-col-after = После
rename-col-status = Состояние
rename-status-ok = Будет переименован
rename-status-unchanged = Без изменений
rename-status-invalid = Недопустимое имя
rename-status-collision = Два файла получат одинаковое имя
rename-status-exists = Файл с таким именем уже есть
rename-invalid-empty = Имя окажется пустым
rename-invalid-path-separator = Имя не может содержать разделитель папок
rename-invalid-dot-name = Имя не может быть . или ..
rename-invalid-forbidden-character = Имя содержит недопустимый символ
rename-invalid-reserved-name = Имя зарезервировано операционной системой
rename-invalid-bad-pattern = Шаблон поиска недопустим
rename-summary = Будет переименовано { $count } файлов.
rename-blocked = Исправьте выделенные строки перед переименованием. До этого ничего не изменится.
rename-apply = Переименовать
rename-toast-done = Переименовано { $count } файлов
rename-nothing-selected = Сначала выберите результаты.
ops-toast-undone = Отменено — возвращено { $count } файлов
ops-toast-redone = Повторено — снова переименовано { $count } файлов
about-portable-on = Портативный режим включён
about-portable-hint = Индекс, настройки и закладки остаются в этой папке. На этот компьютер ничего не записывается.
settings-results-natural-sort = Естественная сортировка (file2 перед file10)
menu-search-ignore-punctuation = Игнорировать пунктуацию
menu-search-ignore-whitespace = Игнорировать пробелы
regex-builder-title = Конструктор регулярных выражений
regex-builder-placeholder = Введите шаблон, например ^отчёт-\d+
regex-builder-match-count = Совпадает { $matched } из { $total } показанных имён
regex-builder-no-results = Сначала выполните поиск — конструктор проверяет шаблон на именах с экрана.
regex-builder-cheatsheet = Шпаргалка
regex-builder-flavor-note = Синтаксис Rust. Без обратных ссылок и просмотра назад — поисковый движок их не поддерживает.
regex-builder-use = Использовать этот шаблон
regex-cheat-any = Любой один символ
regex-cheat-classes = Цифра, символ слова, пробел
regex-cheat-set = Один из набора или ни один из набора
regex-cheat-repeat = Ноль или больше, один или больше, необязательно
regex-cheat-count = Ровно, не менее или диапазон
regex-cheat-anchor = Начало и конец имени
regex-cheat-alt = Одно или другое
regex-cheat-inline-flag = Дальше игнорировать регистр
regex-cheat-noncapturing = Группировать без захвата
regex-cheat-escape = Экранировать специальный символ
quicklook-title = Быстрый просмотр
quicklook-position = { $at } из { $total }
quicklook-hint = Стрелки перемещают по результатам · Пробел или Esc закрывает
quicklook-open-native = Открыть в Быстром просмотре macOS
menu-view-sidebar = Боковая панель
sidebar-title = Боковая панель
sidebar-bookmarks = Закладки
sidebar-filters = Фильтры
sidebar-volumes = Тома
sidebar-recent = Недавние запросы
sidebar-no-bookmarks = Закладок пока нет
sidebar-no-volumes = Нет проиндексированных томов
sidebar-no-recent = Пока пусто
sidebar-clear-recent = Очистить
menu-tools-permission-health = Состояние доступа…
permissions-title = Состояние доступа
permissions-all-clear = Всё, что вы попросили проиндексировать, удалось прочитать.
permissions-summary = { $denied } папок не удалось прочитать из-за отказа в доступе, ещё { $other } не удалось по другим причинам. Файлы внутри них не попали в индекс и не появятся в результатах.
permissions-truncated = Ещё { $dropped } учтено, но не показано.
permissions-how-to-fix = Как это исправить
permissions-by-volume = Где
permissions-group-counts = { $denied } отказов · { $other } прочих
permissions-macos-granted = У Freally уже есть полный доступ к диску. Папки ниже защищены чем-то другим — проверьте их владельца и права.
permissions-macos-steps = macOS защищает эти папки, пока вы не предоставите полный доступ к диску. Откройте «Конфиденциальность и безопасность» → «Полный доступ к диску», включите Freally, закройте и снова откройте Freally и повторите сканирование.
permissions-macos-open-settings = Открыть настройки полного доступа к диску
permissions-macos-undetermined = Freally не смог определить, предоставлен ли уже полный доступ к диску, так что причина может быть иной.
permissions-linux-steps = Ваш пользователь не может прочитать эти пути. Проверьте владельца и права папки или смонтируйте файловую систему с правами, через которые ваша учётная запись может пройти. Для системных путей службе может потребоваться запуск с повышенными правами.
permissions-windows-steps = Правило доступа Windows запрещает вашей учётной записи. Откройте «Свойства» → «Безопасность» папки, чтобы увидеть правило, или запустите службу индексирования от имени пользователя, который может её читать.
permissions-generic-steps = Операционная система отказалась читать эти пути. Проверьте их права.
status-paths-skipped = пропущено папок: { $count }
media-open-externally = Открыть в системном плеере
media-play-pause = Воспроизвести или пауза
media-seek = Перемотка
media-loop = Повтор
