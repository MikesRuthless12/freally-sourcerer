...
dupe-group-summary = { $count } cópias · { $bytes }
status-file-list = Lista de arquivos: { $name }
status-file-list-truncated = Mostrando as primeiras 5.000 entradas
refine-label = Nos resultados
refine-input-label = Restringir os resultados atuais
refine-placeholder = Digite para restringir, Enter para adicionar
refine-count = { $shown } de { $total }
refine-remove-chip = Remover o filtro { $term }
refine-clear = Limpar
refine-close = Fechar a barra de refinamento
menu-search-within-results = Pesquisar nos resultados
ctx-menu-label = Ações do resultado
ctx-open = Abrir
ctx-reveal = Mostrar no gerenciador de arquivos
ctx-view-hits = Ver ocorrências em contexto
ctx-open-with = Abrir com
ctx-open-with-none = Nenhum aplicativo registrado
ctx-loading = Carregando…
ctx-copy-contents = Copiar conteúdo
ctx-copy-as-file = Copiar como arquivo
ctx-copy-path-list = Copiar lista de caminhos
ctx-quote-lines = Um por linha
ctx-quote-quoted = Entre aspas
ctx-quote-space = Separados por espaços
ctx-quote-escaped = Escapados para shell
ctx-terminal-here = Abrir terminal aqui
verb-failed = Falha em { $verb }: { $error }
hitviewer-title = Ocorrências em { $name }
hitviewer-no-matches = Nenhuma ocorrência
hitviewer-match-position = Ocorrência { $current } de { $total }
hitviewer-prev = Anterior
hitviewer-next = Próxima
hitviewer-loading = Extraindo…
hitviewer-extractor = Extraído por { $id }
hitviewer-truncated = Documento truncado para exibição
hitviewer-keys = F3 próxima · Shift+F3 anterior · Esc fechar
close = Fechar
toast-dismiss = Dispensar
toast-file-list-opened = { $name } aberto — { $count } entradas
toast-file-list-open-failed = Não foi possível abrir a lista de arquivos: { $error }
toast-export-done = { $written } resultados exportados como { $format }
toast-export-lossy = { $written } de { $total } resultados exportados como { $format }
toast-export-failed = Falha na exportação: { $error }
toast-search-failed = Falha na busca: { $error }
toast-settings-save-failed = Não foi possível salvar as configurações: { $error }
toast-settings-reset-failed = Não foi possível restaurar os padrões: { $error }
toast-index-root-failed = Não foi possível adicionar { $path } ao índice: { $error }
menu-help-report-bug = Relatar um problema…
bugreport-title = Relatar um problema
bugreport-crash-notice = O Freally Sourcerer fechou inesperadamente na última execução. Você pode enviar um relatório anônimo sobre isso.
bugreport-what-happened = O que aconteceu?
bugreport-placeholder = O que você estava fazendo quando deu errado?
bugreport-include-crash = Incluir os detalhes da falha abaixo
bugreport-exact-text = É exatamente isto que será enviado
bugreport-privacy = Nada é enviado automaticamente. Isto abre um rascunho já preenchido — você lê e envia por conta própria. O relatório traz apenas a versão do app e o seu sistema operacional; sua pasta pessoal e nome de usuário são removidos.
bugreport-github = Abrir uma issue no GitHub
bugreport-gmail = Abrir no Gmail
bugreport-email = Abrir no meu app de e-mail
bugreport-copy = Copiar relatório
bugreport-copied = Copiado.
bugreport-discard-crash = Descartar este relatório de falha
bugreport-opened = Há um rascunho aberto. Leia e envie você mesmo.
bugreport-failed = Não foi possível abrir o rascunho: { $error }
logs-crash-drills = Testes de relatório de falha
logs-simulate-crash = Simular um relatório de falha
logs-force-crash = Forçar uma falha de teste e sair
updates-check-now = Procurar atualizações agora
updates-checking = Verificando…
updates-last-checked = Última verificação
updates-never-checked = Nunca
updates-current = Você está usando a versão mais recente ({ $current }).
updates-available = A versão { $version } está disponível. Você está usando { $current }.
updates-confirm-title = Instalar atualização
updates-confirm-body = O Freally Sourcerer { $version } está disponível.\n\nVocê está usando { $current }. Deseja fechar o Freally Sourcerer e instalar a nova versão agora?
updates-confirm-yes = Sim, instalar
updates-confirm-no = Agora não
updates-installing = Baixando e instalando { $version }… O Freally Sourcerer será fechado.
updates-failed = Falha na atualização: { $error }
updates-notes-heading = Novidades
toast-copied-paths = { $count } caminhos copiados
toast-copied-contents = Conteúdo de { $name } copiado
toast-copied-files = { $count } arquivos copiados
settings-node-custom-commands = Comandos personalizados
settings-custom-commands-hint = Os comandos adicionados aqui aparecem no menu de contexto dos resultados. Use { "{" }path{ "}" }, { "{" }dir{ "}" }, { "{" }name{ "}" }, { "{" }stem{ "}" } e { "{" }ext{ "}" } nos argumentos.
settings-custom-commands-empty = Ainda não há comandos personalizados.
settings-custom-commands-unnamed = Comando sem nome
settings-custom-commands-name = Nome
settings-custom-commands-program = Programa
settings-custom-commands-args = Argumentos
settings-custom-commands-extensions = Apenas para as extensões
settings-custom-commands-add = Adicionar comando
did-you-mean-label = Você quis dizer
menu-search-match-phonetic = Corresponder fonética CJK

# SRC-M13 — Index Health panel + rebuild advisor
menu-tools-index-health = Saúde do índice…
health-title = Saúde do índice
health-loading = Lendo a saúde do índice…
health-all-good = Todos os locais monitorados estão atualizados.
health-no-watched-roots = Nenhuma pasta está sendo indexada ainda.
health-col-volume = Local
health-col-status = Status
health-col-lag = Atraso
health-col-last-event = Última alteração
health-col-applied = Aplicados
health-col-dropped = Descartados
health-col-queue = Fila
health-status-live = Ao vivo
health-status-scan-only = Somente varredura
health-lag-value = { $last } ms (pico { $max } ms)
health-no-events-yet = Nenhuma alteração ainda
health-never = Nunca
health-ago = há { $secs } s
health-legend = O atraso é o tempo entre uma alteração no disco e sua aparição nos resultados. “Aplicados” conta os eventos indexados, com os eventos mesclados entre parênteses.
health-extraction-untracked = A fila de extração de conteúdo ainda não é monitorada.
health-extraction-backlog = Fila de extração de conteúdo: { $count }
health-fix-rebuild = Reconstruir índice
health-advice-journal-stream-reset = O sistema operacional descartou o fluxo de alterações em { $root }. As alterações feitas nesse intervalo faltam no índice — uma reconstrução as restaura.
health-advice-events-dropped = { $count } alterações em { $root } chegaram mais rápido do que podiam ser indexadas e foram descartadas. Uma reconstrução as restaura.
health-advice-not-monitoring = As alterações em { $root } não estão sendo monitoradas, então os resultados só são atualizados quando a pasta é varrida novamente.
health-advice-high-lag = As alterações em { $root } estão levando até { $count } ms para aparecer nos resultados.
health-advice-queue-saturated = A fila de alterações de { $root } está quase cheia ({ $count } aguardando); alterações podem começar a ser descartadas.

# SRC-M14 — offline removable-volume catalogs
results-offline-badge = off-line — { $name }
results-offline-badge-title = A unidade onde este arquivo está não está conectada. A entrada vem do catálogo que o Freally mantém desse dispositivo.

# SRC-M15 bulk rename + SRC-M16 undo/redo
menu-edit-undo = Desfazer
menu-edit-redo = Refazer
menu-edit-bulk-rename = Renomear…
rename-title = Renomear { $count } selecionados
rename-find = Localizar
rename-replace = Substituir por
rename-use-regex = Expressão regular
rename-ignore-case = Ignorar maiúsculas
rename-part = Aplicar a
rename-part-stem = Nome sem extensão
rename-part-full = Nome completo
rename-case = Maiúsculas
rename-case-none = Sem alteração
rename-case-lower = minúsculas
rename-case-upper = MAIÚSCULAS
rename-case-title = Iniciais Maiúsculas
rename-counter-start = Contador a partir de
rename-hint = Use { "{" }n{ "}" } para um contador e { "{" }n:03{ "}" } para preenchê-lo. Com expressões regulares, $1 insere um grupo.
rename-col-before = Antes
rename-col-after = Depois
rename-col-status = Status
rename-status-ok = Será renomeado
rename-status-unchanged = Sem alteração
rename-status-invalid = Nome inválido
rename-status-collision = Dois arquivos ficariam com o mesmo nome
rename-status-exists = Já existe um arquivo com esse nome
rename-invalid-empty = O nome ficaria vazio
rename-invalid-path-separator = O nome não pode conter separador de pasta
rename-invalid-dot-name = O nome não pode ser . ou ..
rename-invalid-forbidden-character = O nome contém um caractere não permitido
rename-invalid-reserved-name = O nome é reservado pelo sistema operacional
rename-invalid-bad-pattern = O padrão de busca não é válido
rename-summary = { $count } arquivos serão renomeados.
rename-blocked = Corrija as linhas destacadas antes de renomear. Até lá, nada é alterado.
rename-apply = Renomear
rename-toast-done = { $count } arquivos renomeados
rename-nothing-selected = Selecione alguns resultados primeiro.
ops-toast-undone = Desfeito — { $count } arquivos restaurados
ops-toast-redone = Refeito — { $count } arquivos renomeados novamente
about-portable-on = O modo portátil está ativado
about-portable-hint = Seu índice, configurações e favoritos ficam nesta pasta. Nada é gravado neste computador.
settings-results-natural-sort = Ordenação natural (file2 antes de file10)
menu-search-ignore-punctuation = Ignorar pontuação
menu-search-ignore-whitespace = Ignorar espaços
regex-builder-title = Construtor de expressões regulares
regex-builder-placeholder = Digite um padrão, ex. ^relatorio-\d+
regex-builder-match-count = { $matched } de { $total } nomes exibidos correspondem
regex-builder-no-results = Faça uma busca primeiro — o construtor testa seu padrão nos nomes na tela.
regex-builder-cheatsheet = Referência rápida
regex-builder-flavor-note = Sintaxe Rust. Sem retrorreferências e sem lookaround — o motor de busca não os suporta.
regex-builder-use = Usar este padrão
regex-cheat-any = Qualquer caractere
regex-cheat-classes = Dígito, caractere de palavra, espaço
regex-cheat-set = Um do conjunto, ou nenhum do conjunto
regex-cheat-repeat = Zero ou mais, um ou mais, opcional
regex-cheat-count = Exatamente, no mínimo, ou um intervalo
regex-cheat-anchor = Início e fim do nome
regex-cheat-alt = Um ou outro
regex-cheat-inline-flag = Ignorar maiúsculas a partir daqui
regex-cheat-noncapturing = Agrupar sem capturar
regex-cheat-escape = Escapar um caractere especial
quicklook-title = Visualização rápida
quicklook-position = { $at } de { $total }
quicklook-hint = As setas percorrem os resultados · Espaço ou Esc fecha
quicklook-open-native = Abrir na Visualização rápida do macOS
...
sidebar-recent = Buscas recentes
sidebar-no-bookmarks = Nenhum favorito ainda
sidebar-no-volumes = Nenhum volume indexado
sidebar-no-recent = Nada ainda
sidebar-clear-recent = Limpar
menu-tools-permission-health = Estado das permissões…
permissions-title = Estado das permissões
permissions-all-clear = Tudo o que você pediu para indexar pôde ser lido.
permissions-summary = { $denied } pastas não puderam ser lidas por acesso negado e { $other } falharam por outros motivos. Os arquivos dentro delas não estão no índice e não aparecerão nos resultados.
permissions-truncated = Mais { $dropped } foram contadas, mas não listadas.
permissions-how-to-fix = Como resolver
permissions-by-volume = Onde
permissions-group-counts = { $denied } negadas · { $other } outras
permissions-macos-granted = O Freally já tem Acesso Total ao Disco. As pastas abaixo estão protegidas por outra coisa — verifique o proprietário e as permissões.
permissions-macos-steps = O macOS protege estas pastas até você conceder Acesso Total ao Disco. Abra Privacidade e Segurança → Acesso Total ao Disco, ative o Freally, feche e reabra o Freally e faça uma nova varredura.
permissions-macos-open-settings = Abrir ajustes de Acesso Total ao Disco
permissions-macos-undetermined = O Freally não conseguiu determinar se o Acesso Total ao Disco já foi concedido, então talvez o problema seja outro.
permissions-linux-steps = Seu usuário não consegue ler estes caminhos. Verifique o proprietário e o modo da pasta, ou monte o sistema de arquivos com permissões que sua conta possa percorrer. Indexar caminhos do sistema pode exigir que o daemon seja executado com privilégios elevados.
permissions-windows-steps = Uma regra de acesso do Windows nega sua conta. Abra Propriedades → Segurança da pasta para ver a regra aplicada, ou execute o serviço de índice como um usuário que possa lê-la.
permissions-generic-steps = O sistema operacional recusou-se a ler estes caminhos. Verifique as permissões.
status-paths-skipped = { $count } pastas ignoradas
media-open-externally = Abrir no reprodutor do sistema
media-play-pause = Reproduzir ou pausar
media-seek = Avançar
media-loop = Repetir
media-volume = Volume
