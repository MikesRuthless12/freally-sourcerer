...
toast-search-failed = 検索に失敗しました: { $error }
toast-settings-save-failed = 設定を保存できませんでした: { $error }
toast-settings-reset-failed = 既定値に戻せませんでした: { $error }
toast-index-root-failed = { $path } をインデックスに追加できませんでした: { $error }
menu-help-report-bug = バグを報告…
bugreport-title = バグを報告
bugreport-crash-notice = 前回の実行時に Freally Sourcerer が予期せず終了しました。匿名のレポートを送信できます。
bugreport-what-happened = 何が起きましたか？
bugreport-placeholder = 問題が起きたとき、何をしていましたか？
bugreport-include-crash = 下のクラッシュ詳細を含める
bugreport-exact-text = 送信されるのはこの内容です
bugreport-privacy = 自動送信は行いません。下書きが開くので、内容を確認してご自身で送信してください。レポートに含まれるのはアプリのバージョンと OS だけで、ホームフォルダーとユーザー名は取り除かれます。
bugreport-github = GitHub で issue を開く
bugreport-gmail = Gmail で開く
bugreport-email = メールアプリで開く
bugreport-copy = レポートをコピー
bugreport-copied = コピーしました。
bugreport-discard-crash = このクラッシュレポートを破棄
bugreport-opened = 下書きを開きました。確認のうえ、ご自身で送信してください。
bugreport-failed = 下書きを開けませんでした: { $error }
logs-crash-drills = クラッシュレポートの動作確認
logs-simulate-crash = クラッシュレポートをシミュレート
logs-force-crash = テスト用にクラッシュさせて終了
updates-check-now = 今すぐ更新を確認
updates-checking = 確認中…
updates-last-checked = 最終確認
updates-never-checked = 未確認
updates-current = 最新リリース ({ $current }) を使用中です。
updates-available = バージョン { $version } が利用できます。現在は { $current } です。
updates-confirm-title = 更新をインストール
updates-confirm-body = Freally Sourcerer { $version } が利用できます。\n\n現在は { $current } です。Freally Sourcerer を閉じて新しいバージョンを今すぐインストールしますか？
updates-confirm-yes = はい、インストールします
updates-confirm-no = 後で
updates-installing = { $version } をダウンロードしてインストールしています… Freally Sourcerer は終了します。
updates-failed = 更新に失敗しました: { $error }
updates-notes-heading = 変更点
...
menu-tools-index-health = インデックスの状態…
health-title = インデックスの状態
health-loading = インデックスの状態を読み込み中…
health-all-good = 監視中のすべての場所は最新です。
health-no-watched-roots = まだインデックス対象のフォルダーがありません。
health-col-volume = 場所
health-col-status = 状態
health-col-lag = 遅延
health-col-last-event = 最終変更
health-col-applied = 適用
health-col-dropped = 破棄
health-col-queue = キュー
health-status-live = ライブ
health-status-scan-only = スキャンのみ
health-lag-value = { $last } ms（最大 { $max } ms）
health-no-events-yet = 変更はまだありません
health-never = なし
health-ago = { $secs } 秒前
health-legend = 遅延は、ディスク上の変更が結果に現れるまでの時間です。「適用」はインデックスされたイベント数で、括弧内は統合されたイベント数です。
health-extraction-untracked = コンテンツ抽出の滞留はまだ計測していません。
health-extraction-backlog = コンテンツ抽出の滞留: { $count }
health-fix-rebuild = インデックスを再構築
health-advice-journal-stream-reset = OS が { $root } の変更ストリームを破棄しました。その間の変更はインデックスにありません。再構築すると復元されます。
health-advice-events-dropped = { $root } の変更 { $count } 件がインデックス処理に追いつかず破棄されました。再構築すると復元されます。
...
menu-edit-undo = 元に戻す
menu-edit-redo = やり直す
menu-edit-bulk-rename = 名前を変更…
rename-title = 選択した { $count } 件の名前を変更
rename-find = 検索
rename-replace = 置換後
rename-use-regex = 正規表現
rename-ignore-case = 大文字と小文字を区別しない
rename-part = 適用対象
rename-part-stem = 拡張子を除く名前
rename-part-full = ファイル名全体
rename-case = 大文字・小文字
rename-case-none = 変更しない
rename-case-lower = 小文字
rename-case-upper = 大文字
rename-case-title = 各単語の先頭を大文字
rename-counter-start = 連番の開始
rename-hint = { "{" }n{ "}" } で連番、{ "{" }n:03{ "}" } で桁揃えします。正規表現では $1 でキャプチャを挿入できます。
rename-col-before = 変更前
rename-col-after = 変更後
rename-col-status = 状態
rename-status-ok = 変更されます
rename-status-unchanged = 変更なし
rename-status-invalid = 無効な名前
rename-status-collision = 2 つのファイルが同じ名前になります
rename-status-exists = 同名のファイルが既にあります
rename-invalid-empty = 名前が空になります
rename-invalid-path-separator = 名前にフォルダー区切り文字は使えません
rename-invalid-dot-name = 名前を . や .. にはできません
rename-invalid-forbidden-character = 使用できない文字が含まれています
rename-invalid-reserved-name = OS が予約している名前です
rename-invalid-bad-pattern = 検索パターンが正しくありません
rename-summary = { $count } 件のファイル名を変更します。
rename-blocked = 強調された行を修正してください。それまで何も変更されません。
rename-apply = 名前を変更
rename-toast-done = { $count } 件の名前を変更しました
rename-nothing-selected = 先に結果を選択してください。
ops-toast-undone = 元に戻しました — { $count } 件を復元
ops-toast-redone = やり直しました — { $count } 件を再度変更
about-portable-on = ポータブルモードが有効です
about-portable-hint = インデックス、設定、ブックマークはこのフォルダーに保存されます。このコンピューターには何も書き込まれません。
settings-results-natural-sort = 自然順ソート（file10 より file2 が先）
menu-search-ignore-punctuation = 句読点を無視
menu-search-ignore-whitespace = 空白を無視
regex-builder-title = 正規表現ビルダー
regex-builder-placeholder = パターンを入力（例: ^report-\d+）
regex-builder-match-count = 表示中の { $total } 件のうち { $matched } 件が一致
regex-builder-no-results = 先に検索してください。ビルダーは画面上の名前でパターンを試します。
regex-builder-cheatsheet = チートシート
regex-builder-flavor-note = Rust の構文です。後方参照と先読み・後読みは使えません（検索エンジンが対応していません）。
regex-builder-use = このパターンを使う
regex-cheat-any = 任意の 1 文字
regex-cheat-classes = 数字、単語構成文字、空白
regex-cheat-set = 集合のいずれか、または集合以外
regex-cheat-repeat = 0 回以上、1 回以上、省略可
regex-cheat-count = ちょうど、以上、または範囲
regex-cheat-anchor = 名前の先頭と末尾
regex-cheat-alt = どちらか一方
regex-cheat-inline-flag = ここ以降は大文字小文字を無視
regex-cheat-noncapturing = キャプチャせずにグループ化
regex-cheat-escape = 特殊文字をエスケープ
quicklook-title = クイックルック
quicklook-position = { $total } 件中 { $at } 件目
quicklook-hint = 矢印キーで結果を移動 · スペースまたは Esc で閉じる
quicklook-open-native = macOS のクイックルックで開く
menu-view-sidebar = サイドバー
sidebar-title = サイドバー
sidebar-bookmarks = ブックマーク
sidebar-filters = フィルター
sidebar-volumes = ボリューム
sidebar-recent = 最近の検索
sidebar-no-bookmarks = ブックマークはまだありません
sidebar-no-volumes = インデックス済みのボリュームがありません
sidebar-no-recent = まだありません
sidebar-clear-recent = 消去
menu-tools-permission-health = アクセス権の状態…
permissions-title = アクセス権の状態
permissions-all-clear = インデックス対象として指定したものはすべて読み取れました。
permissions-summary = アクセスが拒否されたため { $denied } 個のフォルダーを読み取れず、{ $other } 個は別の理由で失敗しました。その中のファイルはインデックスに含まれず、検索結果にも表示されません。
permissions-truncated = ほかに { $dropped } 件を数えましたが、一覧には含めていません。
permissions-how-to-fix = 対処方法
permissions-by-volume = 場所
permissions-group-counts = 拒否 { $denied } 件 · その他 { $other } 件
permissions-macos-granted = Freally にはすでにフルディスクアクセスがあります。以下のフォルダーは別の仕組みで保護されています。所有者とアクセス権を確認してください。
permissions-macos-steps = フルディスクアクセスを許可するまで、macOS はこれらのフォルダーを保護します。「プライバシーとセキュリティ」→「フルディスクアクセス」を開いて Freally をオンにし、Freally を終了して開き直してから再スキャンしてください。
permissions-macos-open-settings = フルディスクアクセスの設定を開く
permissions-macos-undetermined = フルディスクアクセスがすでに許可されているかを判定できませんでした。原因は別かもしれません。
permissions-linux-steps = お使いのユーザーではこれらのパスを読み取れません。フォルダーの所有者とモードを確認するか、アカウントが辿れる権限でファイルシステムをマウントしてください。システム全体のパスをインデックスするには、デーモンを昇格した権限で実行する必要がある場合があります。
permissions-windows-steps = Windows のアクセス規則がこのアカウントを拒否しています。フォルダーの「プロパティ」→「セキュリティ」で適用中の規則を確認するか、読み取れるユーザーでインデックスサービスを実行してください。
permissions-generic-steps = OS がこれらのパスの読み取りを拒否しました。アクセス権を確認してください。
status-paths-skipped = { $count } 個のフォルダーをスキップ
media-open-externally = システムのプレーヤーで開く
media-play-pause = 再生／一時停止
media-seek = シーク
media-loop = ループ
media-volume = 音量
