...
toast-search-failed = Tìm kiếm thất bại: { $error }
toast-settings-save-failed = Không thể lưu cài đặt: { $error }
toast-settings-reset-failed = Không thể khôi phục mặc định: { $error }
toast-index-root-failed = Không thể thêm { $path } vào chỉ mục: { $error }
menu-help-report-bug = Báo lỗi…
bugreport-title = Báo lỗi
bugreport-crash-notice = Freally Sourcerer đã đóng bất ngờ trong lần chạy trước. Bạn có thể gửi một báo cáo ẩn danh về việc này.
bugreport-what-happened = Chuyện gì đã xảy ra?
bugreport-placeholder = Bạn đang làm gì khi sự cố xảy ra?
bugreport-include-crash = Kèm theo chi tiết sự cố bên dưới
bugreport-exact-text = Đây chính xác là nội dung sẽ được gửi
bugreport-privacy = Không có gì được gửi tự động. Thao tác này mở một bản nháp đã điền sẵn — bạn đọc rồi tự gửi. Báo cáo chỉ mang phiên bản ứng dụng và hệ điều hành của bạn; thư mục nhà và tên người dùng đã được loại bỏ.
bugreport-github = Mở một issue trên GitHub
bugreport-gmail = Mở trong Gmail
bugreport-email = Mở trong ứng dụng email của tôi
bugreport-copy = Sao chép báo cáo
bugreport-copied = Đã sao chép.
bugreport-discard-crash = Bỏ báo cáo sự cố này
bugreport-opened = Bản nháp đã mở. Hãy đọc rồi tự gửi.
bugreport-failed = Không thể mở bản nháp: { $error }
logs-crash-drills = Diễn tập báo cáo sự cố
logs-simulate-crash = Mô phỏng một báo cáo sự cố
logs-force-crash = Buộc gây sự cố thử nghiệm và thoát
updates-check-now = Kiểm tra cập nhật ngay
updates-checking = Đang kiểm tra…
updates-last-checked = Lần kiểm tra gần nhất
updates-never-checked = Chưa bao giờ
updates-current = Bạn đang dùng bản phát hành mới nhất ({ $current }).
updates-available = Đã có phiên bản { $version }. Bạn đang dùng { $current }.
updates-confirm-title = Cài đặt bản cập nhật
updates-confirm-body = Đã có Freally Sourcerer { $version }.\n\nBạn đang dùng { $current }. Bạn có muốn đóng Freally Sourcerer và cài đặt phiên bản mới ngay bây giờ không?
updates-confirm-yes = Có, cài đặt
updates-confirm-no = Để sau
updates-installing = Đang tải và cài đặt { $version }… Freally Sourcerer sẽ đóng.
updates-failed = Cập nhật thất bại: { $error }
updates-notes-heading = Có gì mới
...
menu-tools-index-health = Tình trạng chỉ mục…
health-title = Tình trạng chỉ mục
health-loading = Đang đọc tình trạng chỉ mục…
health-all-good = Mọi vị trí được theo dõi đều đã cập nhật.
health-no-watched-roots = Chưa có thư mục nào được lập chỉ mục.
health-col-volume = Vị trí
health-col-status = Trạng thái
health-col-lag = Độ trễ
health-col-last-event = Thay đổi gần nhất
health-col-applied = Đã áp dụng
health-col-dropped = Bị bỏ
health-col-queue = Hàng đợi
health-status-live = Trực tiếp
health-status-scan-only = Chỉ quét
health-lag-value = { $last } ms (đỉnh { $max } ms)
health-no-events-yet = Chưa có thay đổi
health-never = Chưa bao giờ
health-ago = { $secs } giây trước
health-legend = Độ trễ là khoảng thời gian từ khi có thay đổi trên đĩa đến khi nó xuất hiện trong kết quả. “Đã áp dụng” đếm các sự kiện đã lập chỉ mục, sự kiện được gộp nằm trong ngoặc.
health-extraction-untracked = Tồn đọng trích xuất nội dung chưa được theo dõi.
health-extraction-backlog = Tồn đọng trích xuất nội dung: { $count }
health-fix-rebuild = Dựng lại chỉ mục
health-advice-journal-stream-reset = Hệ điều hành đã loại bỏ luồng thay đổi trên { $root }. Những thay đổi trong khoảng đó không có trong chỉ mục — dựng lại sẽ khôi phục chúng.
health-advice-events-dropped = { $count } thay đổi trên { $root } đến nhanh hơn khả năng lập chỉ mục và đã bị bỏ. Dựng lại sẽ khôi phục chúng.
...
menu-edit-bulk-rename = Đổi tên…
rename-title = Đổi tên { $count } mục đã chọn
rename-find = Tìm
rename-replace = Thay bằng
rename-use-regex = Biểu thức chính quy
rename-ignore-case = Bỏ qua hoa thường
rename-part = Áp dụng cho
rename-part-stem = Tên không có phần mở rộng
rename-part-full = Toàn bộ tên tệp
rename-case = Kiểu chữ
rename-case-none = Không đổi
rename-case-lower = chữ thường
rename-case-upper = CHỮ HOA
rename-case-title = Viết Hoa Đầu Từ
rename-counter-start = Bộ đếm từ
rename-hint = Dùng { "{" }n{ "}" } cho bộ đếm và { "{" }n:03{ "}" } để thêm số 0. Với biểu thức chính quy, $1 chèn một nhóm.
rename-col-before = Trước
rename-col-after = Sau
rename-col-status = Trạng thái
rename-status-ok = Sẽ đổi tên
rename-status-unchanged = Không đổi
rename-status-invalid = Tên không hợp lệ
rename-status-collision = Hai tệp sẽ trùng tên
rename-status-exists = Đã có tệp mang tên này
rename-invalid-empty = Tên sẽ bị trống
rename-invalid-path-separator = Tên không được chứa dấu phân cách thư mục
rename-invalid-dot-name = Tên không thể là . hoặc ..
rename-invalid-forbidden-character = Tên chứa ký tự không được phép
rename-invalid-reserved-name = Tên bị hệ điều hành dành riêng
rename-invalid-bad-pattern = Mẫu tìm kiếm không hợp lệ
rename-summary = { $count } tệp sẽ được đổi tên.
rename-blocked = Hãy sửa các dòng được đánh dấu trước khi đổi tên. Cho đến lúc đó không có gì thay đổi.
rename-apply = Đổi tên
rename-toast-done = Đã đổi tên { $count } tệp
rename-nothing-selected = Hãy chọn một số kết quả trước.
ops-toast-undone = Đã hoàn tác — khôi phục { $count } tệp
ops-toast-redone = Đã làm lại — đổi tên lại { $count } tệp
about-portable-on = Chế độ di động đang bật
about-portable-hint = Chỉ mục, cài đặt và dấu trang của bạn ở lại trong thư mục này. Không có gì được ghi vào máy tính này.
settings-results-natural-sort = Sắp xếp tự nhiên (file2 trước file10)
menu-search-ignore-punctuation = Bỏ qua dấu câu
menu-search-ignore-whitespace = Bỏ qua khoảng trắng
regex-builder-title = Trình tạo biểu thức chính quy
regex-builder-placeholder = Nhập một mẫu, ví dụ ^baocao-\d+
regex-builder-match-count = { $matched } trong { $total } tên đang hiển thị khớp
regex-builder-no-results = Hãy tìm kiếm trước — trình tạo thử mẫu của bạn trên các tên đang hiển thị.
regex-builder-cheatsheet = Bảng tra nhanh
regex-builder-flavor-note = Cú pháp Rust. Không có tham chiếu ngược và không có lookaround — công cụ tìm kiếm không hỗ trợ.
regex-builder-use = Dùng mẫu này
regex-cheat-any = Một ký tự bất kỳ
regex-cheat-classes = Chữ số, ký tự từ, khoảng trắng
regex-cheat-set = Một trong tập hợp, hoặc không thuộc tập hợp
regex-cheat-repeat = Không hoặc nhiều, một hoặc nhiều, tùy chọn
regex-cheat-count = Đúng bằng, ít nhất, hoặc một khoảng
regex-cheat-anchor = Đầu và cuối tên
regex-cheat-alt = Cái này hoặc cái kia
regex-cheat-inline-flag = Bỏ qua hoa thường từ đây
regex-cheat-noncapturing = Nhóm mà không bắt giữ
regex-cheat-escape = Thoát một ký tự đặc biệt
quicklook-title = Xem nhanh
quicklook-position = { $at } trên { $total }
quicklook-hint = Phím mũi tên duyệt kết quả · Phím cách hoặc Esc để đóng
quicklook-open-native = Mở bằng Xem nhanh của macOS
menu-view-sidebar = Thanh bên
sidebar-title = Thanh bên
sidebar-bookmarks = Dấu trang
sidebar-filters = Bộ lọc
sidebar-volumes = Ổ đĩa
sidebar-recent = Tìm kiếm gần đây
sidebar-no-bookmarks = Chưa có dấu trang
sidebar-no-volumes = Chưa lập chỉ mục ổ đĩa nào
sidebar-no-recent = Chưa có gì
sidebar-clear-recent = Xóa
menu-tools-permission-health = Tình trạng quyền truy cập…
permissions-title = Tình trạng quyền truy cập
permissions-all-clear = Mọi thứ bạn yêu cầu Freally lập chỉ mục đều đọc được.
permissions-summary = { $denied } thư mục không đọc được vì bị từ chối truy cập, và { $other } thất bại vì lý do khác. Các tệp bên trong không có trong chỉ mục và sẽ không xuất hiện trong kết quả.
permissions-truncated = Còn { $dropped } mục nữa được đếm nhưng không liệt kê.
permissions-how-to-fix = Cách khắc phục
permissions-by-volume = Ở đâu
permissions-group-counts = { $denied } bị từ chối · { $other } khác
permissions-macos-granted = Freally đã có Quyền truy cập toàn bộ đĩa. Các thư mục dưới đây được bảo vệ bởi thứ khác — hãy kiểm tra chủ sở hữu và quyền của chúng.
permissions-macos-steps = macOS bảo vệ các thư mục này cho đến khi bạn cấp Quyền truy cập toàn bộ đĩa. Mở Quyền riêng tư & Bảo mật → Truy cập toàn bộ đĩa, bật Freally, thoát rồi mở lại Freally và quét lại.
permissions-macos-open-settings = Mở cài đặt Truy cập toàn bộ đĩa
permissions-macos-undetermined = Freally không xác định được liệu Quyền truy cập toàn bộ đĩa đã được cấp hay chưa, nên có thể đây không phải nguyên nhân.
permissions-linux-steps = Người dùng của bạn không đọc được các đường dẫn này. Hãy kiểm tra chủ sở hữu và chế độ của thư mục, hoặc gắn kết hệ thống tệp với quyền mà tài khoản của bạn có thể đi qua. Lập chỉ mục các đường dẫn toàn hệ thống có thể cần dịch vụ nền chạy với quyền nâng cao.
permissions-windows-steps = Một quy tắc truy cập của Windows từ chối tài khoản của bạn. Mở Thuộc tính → Bảo mật của thư mục để xem quy tắc nào đang áp dụng, hoặc chạy dịch vụ lập chỉ mục dưới người dùng có quyền đọc.
permissions-generic-steps = Hệ điều hành đã từ chối đọc các đường dẫn này. Hãy kiểm tra quyền của chúng.
status-paths-skipped = bỏ qua { $count } thư mục
media-open-externally = Mở bằng trình phát hệ thống
media-play-pause = Phát hoặc tạm dừng
media-seek = Tua
media-loop = Lặp
