//! Linux inotify+fanotify journal subscriber for Freally (Phase 3).
//!
//! Public surface mirrors `freally-journal-win` (Phase 1) and
//! `freally-journal-mac` (Phase 2) so the facade in
//! `freally-journal` re-exports the same shape on every OS:
//!
//! ```ignore
//! use freally_journal_lin::{open, JournalEvent};
//! use std::path::Path;
//! use futures::StreamExt;
//!
//! # async fn demo() -> Result<(), freally_journal_lin::JournalError> {
...
//! `/usr/share/polkit-1/actions/io.mikeweaver.freally.policy` brokers
//! the elevation.
