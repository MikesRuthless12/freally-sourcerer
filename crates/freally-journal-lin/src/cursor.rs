...
//! diagnostics. Cursors live under `~/.local/share/freally/cursors/` —
...
//! 3. Surface `backend` for diagnostics so a `freally-indexd doctor`
...
impl WatchCursor {
    /// Default cursor root: `$XDG_DATA_HOME/freally/cursors/` (defaults
    /// to `~/.local/share/freally/cursors/` per the XDG basedir spec).
    /// Falls back to `<system_temp>/freally/cursors` when neither
    /// `XDG_DATA_HOME` nor `HOME` is set (e.g. systemd unit contexts that
    /// strip env vars).
    pub fn default_root() -> PathBuf {
        if let Some(xdg) = std::env::var_os("XDG_DATA_HOME") {
            return PathBuf::from(xdg).join("freally").join("cursors");
        }
        ...
        std::env::temp_dir().join("freally").join("cursors")
    }
...
mod tests {
    ...
    fn default_root_prefers_xdg_data_home() {
        ...
        assert_eq!(
            root,
            PathBuf::from("/tmp/xdg-data-home/freally/cursors"),
