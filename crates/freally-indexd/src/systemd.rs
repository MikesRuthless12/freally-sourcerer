//! Linux systemd-user-unit wiring for `freally-indexd`.
//!
//! - `install()` writes the per-user systemd unit and runs
//!   `systemctl --user enable --now freally-indexd.service`. The unit
...
//! `io.mikeweaver.freally.elevate`); see `polkit/io.mikeweaver.
//! freally.policy` for the policy file the installer drops at
...
const SERVICE_NAME: &str = "freally-indexd.service";
...
fn render_unit(binary: &Path, logs_dir: &Path) -> String {
    ...
    // user-supplied `--binary "/path with spaces/freally-indexd"` does
...
mod tests {
    ...
    fn render_unit_contains_required_keys() {
        let unit = render_unit(
            ...
            Path::new("/home/me/.local/share/freally/logs"),
        );
        assert!(unit.contains("Type=simple"));
        assert!(unit.contains("Restart=always"));
        assert!(unit.contains("WantedBy=default.target"));
        assert!(unit.contains("ExecStart=\"/usr/local/bin/freally-indexd\" service"));
    ...
    fn render_unit_quotes_paths_with_spaces() {
        ...
        let unit = render_unit(
            Path::new("/path with spaces/freally-indexd"),
            Path::new("/home/me/.local/share/freally/logs"),
        );
        assert!(unit.contains("ExecStart=\"/path with spaces/freally-indexd\" service"));
    }
    ...
    fn unit_path_uses_home() {
        ...
        assert_eq!(
            path,
            PathBuf::from("/tmp/test-home/.config/systemd/user/freally-indexd.service")
        );
        unsafe {
            match prior {
