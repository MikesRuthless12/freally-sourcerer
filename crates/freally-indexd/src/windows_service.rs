//! Windows Service Control Manager wiring for `freally-indexd`.
//!
//! - `install()`  — registers `Freally-Indexd` with the SCM.
...
const SERVICE_NAME: &str = "Freally-Indexd";
const SERVICE_DISPLAY_NAME: &str = "Freally Indexer";
const SERVICE_DESCRIPTION: &str =
    "Maintains the Freally realtime file index by subscribing to NTFS USN journals.";
...
fn init_service_logging() {
    // PROGRAMDATA\Freally\logs\indexd-service.log. The service runs
    ...
    tracing::info!(
        ...
        "freally-indexd service starting"
    );
}
