//! Freally indexer daemon — library entrypoint.
//!
//! `freally-indexd` runs as a long-lived daemon. The process is
//! invoked four ways:
//!
//! 1. `freally-indexd run` — foreground; for dev / smoke tests.
//! 2. `freally-indexd install` / `uninstall` — register / deregister
//!    with the per-OS service manager (SCM / launchd / systemd).
//! 3. `freally-indexd service` — the entry point that the OS service
//!    manager invokes; runs the daemon under the platform's
//!    service-control conventions.
//! 4. Linked into `freally-ui` as a child process started at app boot;
...
use anyhow::Result;
use freally_rpc::{Server, ServerConfig, SocketPath, default_socket_path};
