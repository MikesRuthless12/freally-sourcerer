...
//! round-trips them through `~/.config/freally/extractors.json`
//! (XDG_CONFIG_HOME on Linux, `%APPDATA%\Freally\` on Windows,
//! `~/Library/Application Support/Freally/` on macOS); the daemon
...
/// Default queue capacity. Mirrors `freally-index::DEFAULT_CAPACITY`
