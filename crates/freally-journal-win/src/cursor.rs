...
//! live under `%LOCALAPPDATA%\Freally\cursors\<volume_serial>.json`.
//!
...
impl VolumeCursor {
    ...
    /// supplied root directory (typically `%LOCALAPPDATA%\Freally\cursors`).
    pub fn path_in(root: &Path, volume_serial: u32) -> PathBuf {
        root.join(format!("{volume_serial:08x}.json"))
    }

    /// Default cursor root: `%LOCALAPPDATA%\Freally\cursors`. Falls back to
    /// `<system_temp>\Freally\cursors` if `LOCALAPPDATA` is unset (e.g.
    /// service contexts that strip env vars).
    pub fn default_root() -> PathBuf {
        if let Some(local) = std::env::var_os("LOCALAPPDATA") {
            PathBuf::from(local).join("Freally").join("cursors")
        } else {
