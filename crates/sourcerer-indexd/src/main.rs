...
//!
//! Phase 2 adds the macOS launchd-agent path: `install` writes a per-user
...
//! smoke-test invocation. Phase 4 fills in the per-root subscriber +
//! index core inside the agent body.
...
fn main() -> Result<()> {
    ...
    match cli.command.unwrap_or(Command::Run {
        ...
        Command::Install { .. } => {
            anyhow::bail!(
                "`install` is not yet implemented on Linux. The systemd user unit \
                 lands in Phase 3."
            );
        }
        #[cfg(windows)]
        ...
        #[cfg(all(not(windows), not(target_os = "macos")))]
        Command::Uninstall => {
            anyhow::bail!("`uninstall` is not yet implemented on Linux (Phase 3).");
        }
        #[cfg(windows)]
        ...
        Command::Service => {
            anyhow::bail!(
                "`service` is invoked by the OS service manager and is not yet \
                 implemented on Linux (Phase 3)."
            );
        }
    }
}
...
fn run_foreground(_volume: Option<String>, _root: Option<String>, _echo: bool) -> Result<()> {
    tracing::info!(
        "sourcerer-indexd Phase 2: foreground run is Windows + macOS only; Linux lands in Phase 3"
    );
    Ok(())
}
