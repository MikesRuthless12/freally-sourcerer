...
//! sole channel between `freally-indexd` (the server) and the
//! Tauri UI plus the `freally` CLI (clients). Auth: file-mode 0600 +
...
//! ## Surface
//!
//! - `dto`: serde-shaped DTOs that mirror `apps/freally-ui/src/lib/
...
//! - `service`: the `Service` trait — `freally-indexd` implements it
