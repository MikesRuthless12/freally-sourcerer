...
/// Standard JSON-RPC error codes plus Freally-specific extensions.
pub mod codes {
    ...
    /// Freally-specific: the daemon refused the call because the index
    /// is currently corrupt and the integrity policy is `Strict`.
    pub const INDEX_INTEGRITY: i32 = -32000;

    /// Freally-specific: the requested volume / folder / extractor is
    /// not registered.
    pub const NOT_FOUND: i32 = -32001;

    /// Freally-specific: the daemon is shutting down.
