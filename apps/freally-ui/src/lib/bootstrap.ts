...
async function bindNativeEvents() {
  ...
  try {
    ...
    // freally:// URL protocol incoming. Phase 11 supports the search shape.
    await listen<string>("url:opened", async (event) => {
      const u = event.payload;
      if (typeof u !== "string" || u.length > 8192) return;
      try {
        const url = new URL(u);
        if (url.protocol !== "freally:") return;
        ...
        // freally://bundle/<encoded-path> is parsed but Phase 11 does not
...
function registerHandlers() {
  ...
  registry.register("edit.advanced.copy_as_bundle_ref", async () => {
    ...
    const refs = selectedPaths().map((p) => {
      const u = new URL("freally://bundle/");
