//! Spawns the `freally-indexd` binary as a sidecar process and opens a
//! `freally-rpc` client to it.
//!
//! Phase 12 architecture (sidecar mode — the Tauri shell never compiles
//! the heavy daemon dependencies):
//!
//! - Boot: spawn `freally-indexd run` as a child process. The Tauri
//!   shell does NOT depend on `freally-indexd` as a Rust crate — only
//!   on `freally-rpc` (transport types + client) and `freally-query`
...
//! - Locate the `freally-indexd` binary via the env override
//!   `FREALLY_INDEXD_BIN`, or by searching `cargo`'s
//!   `target/{debug,release}` next to the running executable, or by
//!   relying on `PATH`. Binding socket / pipe at the per-OS default.
//! - Connect: open one `freally-rpc` client connection that all
...
impl Daemon {
    ...
    pub fn boot(app: &AppHandle) -> anyhow::Result<Self> {
        let runtime = tokio::runtime::Builder::new_multi_thread()
            .enable_all()
            .thread_name("freally-rpc-client")
            .build()?;
        ...
        let bin = locate_indexd_binary().ok_or_else(|| {
            anyhow::anyhow!(
                "could not find `freally-indexd` binary; set `FREALLY_INDEXD_BIN` or place \
                 it next to the running app"
            )
        })?;
        // Pass the socket path explicitly via `--socket` so a smoke test
        // or dev session that overrides `FREALLY_RPC_SOCKET` is honored
    ...
    pub fn call<P, R>(&self, method: &'static str, params: P) -> Result<R, freally_rpc::RpcError>
    where
...
///
///  1. `FREALLY_INDEXD_BIN` env var (smoke tests / dev sessions).
///  2. Sibling of the current Tauri executable (production bundles).
///  3. `target/{debug,release}/freally-indexd[.exe]` walking up from
...
fn locate_indexd_binary() -> Option<PathBuf> {
    if let Ok(v) = std::env::var("FREALLY_INDEXD_BIN") {
