...
#[tauri::command]
pub fn folders_list() -> Result<Vec<freally_rpc::WatchedFolder>, String> {
    daemon_call("folders.list", serde_json::Value::Null)
}

#[tauri::command]
pub fn folders_add(folder: freally_rpc::WatchedFolder) -> Result<(), String> {
    daemon_call_void(
        "folders.add",
...
#[tauri::command]
pub fn folders_update(folder: freally_rpc::WatchedFolder) -> Result<(), String> {
    daemon_call_void(
        "folders.update",
        serde_json::to_value(folder).map_err(|e| e.to_string())?,
    )
